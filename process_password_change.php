...
if ($result->num_rows > 0) {
    ...
    if ($currentPassword == $user['password']) {
        ...
        if ($updateStmt->execute()) {
            ...
            header("Location: password_change_failed.php?error=Error updating password.");
            exit();
        }
    } else {
        header("Location: password_change_failed.php?error=Current password is incorrect.");
        exit();
    }
} else {
    header("Location: password_change_failed.php?error=User not found.");
    exit();
}
