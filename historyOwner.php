...
-->
                                            ...
                                            <div class="m-0">
                                                ...
                                                <div class="mb-10">
                                                    ...
                                                    <a href="/historyOwner.php" class="custom-list d-flex align-items-center px-5 py-1">
                                                        <div class="d-flex flex-column flex-grow-1">
                                                            <!--begin::Link-->
                                                            <span id="history" class="text-black fw-bold bg-white" style="border: 1px solid white;padding: 10px;border-radius: 5px;">هیستوری فرآیندها</span>
                        ...
                        <div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-2 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
                            ...
                            <ul class="breadcrumb fw-bold fs-base my-1">
                                <li class="breadcrumb-item text-muted">
                                    <a href="/contractor.php" class="p-5 btn btn-white text-dark rounded">صفحه نخست</a>
                                </li>
                                <li class="breadcrumb-item p-5 btn btn-outline-white rounded bg-white bg-black text-white" style="border: 1px solid white">هیستوری فرآیند ها</li>
        ...
        INNER JOIN 
            project ON project.id = projectPersonnel.projectID
        INNER JOIN 
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        WHERE 
                            ...
                            if (in_array($permitID_row, $processedPermits)) {
                                ...
                                if ($row["status"] === "Accept" || $row["status"] === "N/A") {
                                    ...
                                    <div class="accordion-item mb-5 col-12 rounded-2">
                                        <div class="accordion-header" id="tab1-heading_' . $permitID_row . '">
                                            <h2 class="accordion-header" id="tab1-heading_' . $permitID_row . '">
                                                <button class="accordion-button bg-black text-white" type="button" data-bs-toggle="collapse" data-bs-target="#tab1-collapse_' . $permitID_row . '" aria-expanded="false" aria-controls="tab1-collapse_' . $permitID_row . '">
                                                     <span class="bg-dark text-white p-3 rounded">' . $permitID_row . '</span> &nbsp;&nbsp;&nbsp;&nbsp;  <strong class="statusPositionHistory ' . ($row['status'] == 'issued' ? 'text-white bg-warning rounded p-3' : ($row['status'] == 'Accept' ? 'text-white bg-success rounded p-2' : ($row['status'] == 'Reject' ? 'text-white bg-danger rounded p-2' : ($row['status'] == 'in progress' ? 'bg-primary text-white p-3 rounded' : '')))) . '">' . $searchStatus . '</strong>   
                                                </button>
                                            </h2>
                                ...
                                try {
                                    ...
                                    // Assume contractor_name is retrieved from session or another method
                                    // session_start();
                                    $contractor_username = $_SESSION['username'];

                                    // Fetch fullname from users table
                                    $sql_fullname = "SELECT fullname FROM users WHERE username = :contractor_username";
                                    $stmt_fullname = $connection->prepare($sql_fullname);
                                    $stmt_fullname->bindParam(':contractor_username', $contractor_username, PDO::PARAM_STR);
                                    $stmt_fullname->execute();
                                    $fullname_result = $stmt_fullname->fetch(PDO::FETCH_ASSOC);
                                    $contractor_fullname = $fullname_result['fullname'];

                                    // Fetch projectID from projectPersonnel where fullname matches
                                    $sql_projectID = "SELECT projectID FROM projectPersonnel WHERE name = :contractor_fullname";
                                    $stmt_projectID = $connection->prepare($sql_projectID);
                                    $stmt_projectID->bindParam(':contractor_fullname', $contractor_fullname, PDO::PARAM_STR);
                                    $stmt_projectID->execute();
                                    $projectID_result = $stmt_projectID->fetch(PDO::FETCH_ASSOC);
                                    $projectID = $projectID_result['projectID'];

                                    // Retrieve projects based on projectID
                                    $sql_projects = "SELECT * FROM project WHERE id = :projectID";
                                    $stmt_projects = $connection->prepare($sql_projects);
                                    $stmt_projects->bindParam(':projectID', $projectID, PDO::PARAM_INT);
                                    $stmt_projects->execute();
                                    $projects = $stmt_projects->fetchAll(PDO::FETCH_ASSOC);

                                    // Process retrieved projects as needed
                                    foreach ($projects as $project) {
                                        // Process each project
                                        // Example: echo $project['project_name'];
                                    }
                                    ...
                                    foreach ($projects as $project) {
                                        ...
                                        echo '<div class="text-right"><p class="titleOfProcess text-black rounded w-25 p-3 bg-white"> فرآیند چک لیست</p></div>';
                                        echo '<div class="bg-white col-12 border border-1 border-white rounded" style="padding: 10px 0px 20px; width: 98%;">';
                                        echo '<table class="table table-bordered table-striped text-center">';
                                        // echo '<thead class="thead-dark"><tr><th>نام و نام خانوادگی</th><th>عنوان شغلی</th><th>وضعیت فرآیند</th><th>وضعیت بازدید</th><th>تاریخ و ساعت آخرین بازدید</th></tr></thead>';
                                        echo '<thead class="thead-dark"><tr><th>نام و نام خانوادگی</th><th>عنوان شغلی</th><th>وضعیت فرآیند</th></tr></thead>';
                                        ...
                                        foreach ($result1 as $row1) {
                                            ...
                                            echo '<td>' . htmlspecialchars($row1['fullname']) . '</td>';
                                            echo '<td>' . htmlspecialchars($row1['job_title']) . '</td>';
                                            if ($row1['action_status'] == 'noaction') {
                                                echo '<td class="text-danger">انجام نشده</td>';
                                            } else {
                                                echo '<td class="bg-success text-black">انجام شده</td>';
                                            }
                                            echo '</tr>';
                                        }
