...
-->
                                            ...
                                            <div class="m-0">
                                                ...
                                                <div class="mb-10">
                                                    ...
                                                    $username = $_SESSION['username'];
                                                    $fullNameQuery = "SELECT u.fullname, u.job_title, p.role as ROLE
                                                                        FROM users u
                                                                        LEFT JOIN projectPersonnel p ON u.id = p.userID
                                                                        WHERE u.job_position = 3 AND u.username = ?";
                                                    $getFullname = $conn->prepare($fullNameQuery);
                                                    $getFullname->bind_param("s", $username);
                                                    $getFullname->execute();
                                                    $getFullnameResult = $getFullname->get_result();
                                                    $rowFullnameResult = $getFullnameResult->fetch_assoc();
                                                    if ($rowFullnameResult) {
                                                        echo '<a href="/historyOwner.php" class="custom-list d-flex align-items-center px-5 py-1">
                                                                    <div class="d-flex flex-column flex-grow-1">
                                                                        <!--begin::Link-->
                                                                        <span id="history" class="text-black fw-bold bg-white with-hover" style="border: 1px solid white;padding: 10px;border-radius: 5px;">هیستوری فرآیندها</span>
            ...
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
                    ...
                    <div class="container-fluid d-flex align-items-center justify-content-between" id="kt_header_container">
                        ...
                        <div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-2 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
                            ...
                            <ul class="breadcrumb fw-bold fs-base my-1">
                                ...
                                <li class="breadcrumb-item p-5 btn rounded bg-black text-white" style="border: 1px solid white">کارتابل تیم کارفرما</li>
                            </ul>
                            <!--end::Breadcrumb-->
                        </div>
                        <!--end::Page title=-->
                ...
                <div class="px-10">
                    <form method="post" action="">
                        <table class="table table-white px-5 rounded-1">
                            <tbody>
                                <tr>
                                    <td colspan="6" class="text-start">
                                        <a href="/owner.php" class="btn btn-danger btn-sm mb-2 text-white">
                                            <img src="./assets/media/icons/reset_form/icons8-form-50.png" alt="Reset the form" style="width: 20px"> حذف فیلتر جستجوی فرم
                                        </a>
                                        <button type="submit" class="btn btn-primary btn-sm mb-2 text-white">
                                            <img src="./assets/media/icons/reset_form/icons8-search-50.png" alt="Search" style="width: 20px"> جستجو
                                        </button>
                                    </td>
                                </tr>
                                ...
                                try {
                                    ...
                                    $searchProjectName = isset($_POST['search_project_name']) ? $_POST['search_project_name'] : '';
                                    $searchTerm = isset($_POST['search_term']) ? $_POST['search_term'] : '';
                                    $searchContractNumber = isset($_POST['search_contract_number']) ? $_POST['search_contract_number'] : '';
...
(
    -- Query for Checklist
    SELECT DISTINCT
        checklist.date_created AS item_date_created,
        checklist.checklistID AS item_id,
        checklist.projectname AS project_name,
        checklist.permit_id AS permit_id,
        checklist.contract_number AS contract_number,
        COALESCE(
            CASE status.status
                WHEN 'issued' THEN 'ایجاد شده'
                WHEN 'Accept' THEN 'تایید شده'
                WHEN 'Reject' THEN 'تایید نشده'
                WHEN 'in progress' THEN 'در حال بررسی'
                ELSE 'No Status'
            END, 'No Status') AS status_text,
        'Checklist' AS item_type
    FROM 
        checklist
    LEFT JOIN 
        (
            -- Get the latest status for each checklistID
            SELECT 
                s.proccessID AS proccess_id, 
                s.status, 
                s.date_created
            FROM 
                status s
            ...
            ON s.proccessID = latest_status.proccessID
            AND s.date_created = latest_status.max_date
        ) AS status 
        ON status.proccess_id COLLATE utf8mb4_unicode_ci = checklist.permit_id COLLATE utf8mb4_unicode_ci
    WHERE 
        checklist.projectname LIKE :searchProjectName
        AND checklist.checklistID LIKE :searchTerm
        AND checklist.contract_number LIKE :searchContractNumber
        AND (:searchType = '' OR :searchType = 'Checklist')
        AND (:searchStatus = '' OR status.status LIKE :searchStatus)
)
UNION ALL
(
    -- Query for Permit
    SELECT DISTINCT
        permits.dateTimeCreated AS item_date_created,
        permits.permit_id AS item_id,
        permits.project_name AS project_name,
        NULL AS permit_id,
        permits.contract_number AS contract_number,
        COALESCE(
            CASE status.status
                WHEN 'issued' THEN 'ایجاد شده'
                WHEN 'Accept' THEN 'تایید شده'
    ...
    LEFT JOIN 
        (
            -- Get the latest status for each permit_id
            SELECT 
                s.proccessID AS proccess_id, 
                s.status, 
                s.date_created
        ...
        ON status.proccess_id COLLATE utf8mb4_unicode_ci = permits.permit_id COLLATE utf8mb4_unicode_ci
    WHERE 
        permits.project_name LIKE :searchProjectName
        AND permits.permit_id LIKE :searchTerm
        AND permits.contract_number LIKE :searchContractNumber
        AND (:searchType = '' OR :searchType = 'Permit')
        AND (:searchStatus = '' OR status.status LIKE :searchStatus)
)
ORDER BY 
    item_date_created DESC;
                    ...
                    ";
                                    ...
                                    // Bind parameters
                                    $stmt->bindValue(':searchProjectName', '%' . $searchProjectName . '%');
                                    $stmt->bindValue(':searchTerm', '%' . $searchTerm . '%');
                                    $stmt->bindValue(':searchContractNumber', '%' . $searchContractNumber . '%');
                                    $stmt->bindValue(':searchType', $searchType);
                                    $stmt->bindValue(':searchStatus', '%' . $searchStatus . '%');
                                        ...
                                        <tr>
                                            <td colspan="7">
                                                <table class="table table-bordered text-center">
                                                    <thead>
                                                        <tr>
                                                            <th scope="col" class="text-white btn-sm text-center border-radius pr-5">
                                                                <input type="text" name="search_project_name" id="search_project_name" class="form-control" placeholder="نام پروژه" value="<?php echo htmlspecialchars(isset($_POST['search_project_name']) ? $_POST['search_project_name'] : ''); ?>">
                                                            </th>
                                                            <th scope="col" class="text-white btn-sm text-center">
                                                                <input type='text' name='search_term' id='search_term' class='form-control' placeholder="شماره پرمیت | شماره چک لیست" value="<?php echo htmlspecialchars(isset($_POST['search_term']) ? $_POST['search_term'] : ''); ?>">
                                                            </th>
                                                            <th scope="col" class="text-white btn-sm text-center">
                                                                <input type='text' name='search_contract_number' id='search_contract_number' class='form-control' placeholder="شماره قرارداد" value="<?php echo htmlspecialchars(isset($_POST['search_contract_number']) ? $_POST['search_contract_number'] : ''); ?>">
                                                            </th>
                                                            <th scope="col" class="text-white btn-sm text-center">
                                                                <input type='text' name='search_type' id='search_type' class='form-control' placeholder="نوع فرآیند" value="<?php echo htmlspecialchars(isset($_POST['search_type']) ? $_POST['search_type'] : ''); ?>">
                                                            </th>
                                                            <th scope="col" class="text-white btn-sm text-center">
                                                                <input name="search_status" id="search_status" type="text" class="form-control" placeholder="وضعیت فرآیند" value="<?php echo htmlspecialchars(isset($_POST['search_status']) ? $_POST['search_status'] : ''); ?>">
                                                            </th>
                                                            <th scope="col" class="text-center text-black btn-sm" style="padding: 19px;color: #a1a5b7 !important;font-size: 15px;">
                                                                تاریخ و زمان ایجاد
                                                            </th>
                                                            <th scope="col" class="text-center text-black btn-sm" style="padding: 19px;color: #a1a5b7 !important;font-size: 15px;">
                                                                عملیات
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($results as $row): ?>
                                                            ...
                                                            $array = explode(' ', $row['item_date_created']);
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td><?php echo ($row['item_type'] == "Permit" ? "پرمیت" : "چک لیست"); ?></td>
                                                                <td><?php echo htmlspecialchars($row['status_text']); ?></td>
                                                                <td><?php echo htmlspecialchars($jalali_date); ?></td>
                                                                <td>
                                                                    <span>
                                                                        <?php
                                                                        $link = ($row['item_type'] == "Permit" ?
                                                                            "../pages/forms/permitItems.php?prmQWERTY=" . htmlspecialchars($row['item_id']) :
                                                                            "../pages/forms/checklistItems.php?prmQWERTY=" . htmlspecialchars($row['item_id']));
                                                                        ?>
                                                                        <a href="<?php echo $link; ?>" class="btn btn-warning btn-sm">
                                                                            <img src="../assets/media/icons/display_form/display-frame.png" style="width:16px;" title="نمایش فرم" />
                                                                        </a>
                                                                        &nbsp;<a href="../pages/forms/progress.php?itemID=<?php echo $row['item_id']; ?>" class="btn btn-primary btn-sm">
                                                                            <img src="../assets/media/icons/progress/work-in-progress.png" style="width:16px;" title="نمایش روند پیگیری افراد پروژه" />
                                                                        </a>
                                                                    </span>
                                                                </td>
                                                            </tr>
                        ...
                        function searchTable() {
                            ...
                            inputs.forEach(function(inputField) {
                                inputField.addEventListener('input', function() {
                                    ...
                                    let columnIndex = inputField.getAttribute('data-column-index');
