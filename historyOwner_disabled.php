<?php
// error_reporting(0);
require('layouts/infrastructure/headerOwner.php');
$title = "Quality Assurance | History Dashboard";
include_once('datetime/jdf.php');
?>
<!DOCTYPE html>
<html lang="fa">
<!--begin::Head-->
...
<body id="kt_body" style="background-image: url(/assets/media/lines.webp);z-index: 99" class="header-fixed header-tablet-and-mobile-fixed aside-fixed aside-secondary-enabled">
    <!--begin::Main-->
    <!--begin::Root-->
...
-->
                                                ...
                                                <div class="mb-10">
                                                    ...
                                                    <a href="/contractor.php" class="custom-list d-flex align-items-center px-5 py-1">
                                                        ...
                                                        <div class="d-flex flex-column flex-grow-1">
                                                            ...
                                                            <!--end::Title-->
                                                            <!--begin::Link-->
                                                            <span class="text-black fw-bold bg-white" style="border: 1px solid white;padding: 10px;border-radius: 5px;"> صفحه نخست</span>
                                                            <!--end::Link-->
                                                        </div>
                                                        <!--begin::Description-->
                                                    </a>
                                                    ...
                                                    <br />
                                                    <!--begin::Item-->
                                                    <a href="/historyContractor.php" class="custom-list d-flex align-items-center px-5 py-1">
                                                        <div class="d-flex flex-column flex-grow-1">
                                                            <!--begin::Link-->
                                                            <span id="history" class="text-white fw-bold bg-black" style="border: 1px solid white;padding: 10px;border-radius: 5px;">هیستوری فرآیندها</span>
            ...
            <!--end::Aside-->
            <!--begin::Wrapper-->
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper" style="background-color: black;">
                <!--begin::Header-->
                <div id="kt_header" class="header" data-kt-sticky="true" data-kt-sticky-name="header" data-kt-sticky-offset="{default: '200px', lg: '300px'}">
                    ...
                    <div class="container-xxl d-flex align-items-center justify-content-between" id="kt_header_container">
                        ...
                        <div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-2 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
                            <!--begin::Heading-->
                            <!-- <h1 class="text-dark fw-bolder my-0 fs-2">داشبورد کاربری ( پیمانکار )</h1> -->
                            <!--end::Heading-->
                            <!--begin::Breadcrumb-->
                            <ul class="breadcrumb fw-bold fs-base my-1">
                                <li class="breadcrumb-item text-muted">
                                    <a href="/contractor.php" class="p-5 btn btn-outline-white bg-white text-black rounded">صفحه نخست</a>
                                </li>
                                <li class="breadcrumb-item p-5 btn btn-outline-white rounded bg-black text-white" style="border: 1px solid white">هیستوری فرآیند ها</li>
                                <!-- Left Side -->
                                <li class="breadcrumb-item p-5 btn btn-outline-white rounded bg-white text-dark text-left" style="position: absolute;left: 35px;" id="backPage"> صفحه قبل</li>
                                <script>
                ...
                <!--end::Header-->
                <!--begin::Content-->
                <div class="px-10 pt-10 text-center text-white h2 fw-bold">
                    <!-- It is about History of the Permits and Checklist Processes in Contractor Cartable -->
                    <?php
                    error_reporting(0);
                    // Database connection parameters
                    ...
                    // Query to retrieve permits and checklists for the current user
                    $sql = "SELECT 
                                users.fullname, 
                                permits.dateTimeCreated, 
                                status.type, 
                                project.*,
                                permits.permit_id, 
                                project.contract_number,
                                project.project_name ,
                                project.id as `projectIDNumber`,
                                status.status as mainStatus,
                                status.type as mainType,
                                checklist.checklist_id,
                                projectPersonnel.projectID,
                                projectPersonnel.userID,
                                projectPersonnel.id
                            FROM 
                                projectPersonnel
                            INNER JOIN 
                                users ON users.id = projectPersonnel.userID
                            INNER JOIN 
                                project ON project.contractor_name = projectPersonnel.name
                            INNER JOIN 
                                permits ON project.project_name = permits.project_name
                            RIGHT JOIN 
                                status ON status.proccessID = permits.permit_id
                            LEFT JOIN 
                                checklist ON permits.permit_id = checklist.permit_id
                            WHERE 
                                users.username = ? 
                            ORDER BY 
                    ...
                    if ($result->num_rows > 0) {
                        // Array to store processed permit_ids
                        $processedPermits = array();
                        // Output data of each row
                        while ($row = $result->fetch_assoc()) {
                            // Display permit details
                            if ($row["mainType"] === "Permit") {
                                $typeOfForm = "پرمیت";
                            } elseif ($row['mainType'] === "Checklist") {
                                $typeOfForm = "چک لیست";
                            }
                            $array = explode(' ', $row['dateTimeCreated']);
                            list($year, $month, $day) = explode('-', $array[0]);
                            list($hour, $minute, $second) = explode(':', $array[1]);
                            $timestamp = mktime($hour, $minute, $second, $month, $day, $year);
                            $jalali_date = jdate("تاریخ:Y/m/d زمان:H:i:s", $timestamp);
                            $permitID_row = $row['permit_id'];
                            $status_row = $row['mainStatus'];

                            if (isset($row['checklist_id'])) {
                                $typeOfProcess = "<span class='text-primary'> چک لیست</span>";
                            } else {
                                $typeOfProcess = "<span class='text-success'> پرمیت</span>";
                            }

                            // Modify the query to retrieve checklist_status
                            $query_checklist = "SELECT distinct
                                                    cs.user_name AS CheckListUsername
                                                    FROM 
                                                    checklistItemsActions cs 
                                                    WHERE 
                                                    cs.permit_id = ?";

                            $stmt_checklist = $conn->prepare($query_checklist);
                            $stmt_checklist->bind_param("s", $row['permit_id']);
                            $stmt_checklist->execute();
                            $result_checklist = $stmt_checklist->get_result();

                            // Initialize array to store checklist statuses
                            $checklist_statuses = array();

                            // Fetch all rows from the result set
                            while ($row_checklist = $result_checklist->fetch_assoc()) {
                                // Add checklist status to the array
                                $checklist_statuses[] = $row_checklist['CheckListUsername'];
                            }

                            // Separate checklistitemsactions
                            $checklistitemsactions = $checklist_statuses;


                            // Modify the query to retrieve permit_status
                            $query_permit = "SELECT DISTINCT
                                                ps.user_id AS PermitUsername 
                                                        FROM 
                                                            permit_status ps 
                                                        WHERE 
                                                            ps.permit_id = ? ";

                            $stmt_permit = $conn->prepare($query_permit);
                            $stmt_permit->bind_param("s", $row['permit_id']);
                            $stmt_permit->execute();
                            $result_permit = $stmt_permit->get_result();

                            // Initialize array to store permit statuses
                            $permit_statuses = array();

                            // Fetch all rows from the result set
                            while ($row_permit = $result_permit->fetch_assoc()) {
                                // Add permit status to the array
                                $permit_statuses[] = $row_permit['PermitUsername'];
                            }
                            // print_r($permit_statuses);
                            // Separate permit_status
                            $permit_status = $permit_statuses;





                            if (in_array($permitID_row, $processedPermits)) {
                                // echo "1";
                            } else {
                                // a query to achieve status from status table :
                                $statusQuery = "SELECT * FROM status WHERE status.proccessID = ? ";
                                $stmtStatusQuery = $conn->prepare($statusQuery);
                                $stmtStatusQuery->bind_param("s", $row['permit_id']);
                                $stmtStatusQuery->execute();
                                $resultStatusQuery = $stmtStatusQuery->get_result();

                                // Initialize an array to hold the results
                                $printableTest = [];

                                while ($rowStatusQuery = $resultStatusQuery->fetch_assoc()) {
                                    // Determine the status type in Persian
                                    $statusMainStatus =
                                        ($rowStatusQuery['status'] == 'issued' ? 'ایجاد شده' : ($rowStatusQuery['status'] == 'Accept' ? 'تایید شده' : ($rowStatusQuery['status'] == 'Reject' ? 'رد شده' : ($rowStatusQuery['status'] == 'in progress' ? 'در حال بررسی' :
                                                        $rowStatusQuery['status'] // Default case if none of the above match
                                                    ))));
                                    $statusMainType =
                                        ($rowStatusQuery['type'] == 'Permit' ? 'پرمیت' : ($rowStatusQuery['type'] == 'Checklist' ? 'چک لیست' :
                                                $rowStatusQuery['type']));
                                    $printableTest[] = '
        <strong class="statusPositionHistory ' .
                                        ($row['mainStatus'] == 'issued' ? 'text-dark bg-white rounded p-2' : ($row['mainStatus'] == 'Accept' ? 'text-dark bg-white rounded p-2' : ($row['mainStatus'] == 'Reject' ? 'text-dark bg-white rounded p-2' : ($row['mainStatus'] == 'in progress' ? 'bg-white text-dark p-2 rounded' : ''))
                                            )) . '">' .
                                        (
                                            $statusMainType == 'چک لیست' ?
                                            ' پرمیت متناظر: <span class="text-white bg-success rounded px-2">انجام شده</span> |' : ($statusMainType == 'پرمیت' ?
                                                ' پرمیت متناظر: <span class="text-white bg-c122334 rounded px-2">در حال بررسی</span> |' :
                                                ' پرمیت متناظر: <span class="text-white bg-dark rounded px-2">نامشخص</span> |')
                                        ) .
                                        " " . $statusMainType . ":&nbsp;" . ($statusMainStatus == "ایجاد شده" ? '<span class="text-white bg-warning rounded px-2">' . $statusMainStatus . '</span>' : ($statusMainStatus == "تایید شده" ? '<span class="text-white bg-success rounded px-2">' . $statusMainStatus . '</span>' : ($statusMainStatus == "رد شده" ? ('<span class="text-white bg-danger rounded px-2">' . $statusMainStatus . '</span>') : ($statusMainStatus == "در حال بررسی" ? ('<span class="text-white bg-c122334 rounded px-2">' . $statusMainStatus . '</span>') : '')))) . '</strong>';
                                }


                                $processedPermits[] = $permitID_row;
                                if ($row["mainStatus"] === "Accept" || $row["mainStatus"] === "N/A") {
                                    $searchStatus = "تایید شده";
                                } else if ($row["mainStatus"] === "Reject") {
                                    $searchStatus = "تایید نشده";
                                } else if ($row["mainStatus"] === "issued") {
                                    $searchStatus = "ایجاد شده";
                                } else if ($row["mainStatus"] === "in progress") {
                                    $searchStatus = "در حال بررسی";
                                }

                                echo '
    <div class="accordion" dir="RTL">
        <div class="accordion-item mb-5 col-12 rounded-2">
            <div class="accordion-header" id="tab1-heading_' . $permitID_row . '">
                <h2 class="accordion-header" id="tab1-heading_' . $permitID_row . '">
                            <button class="accordion-button bg-black text-white" type="button" data-bs-toggle="collapse" data-bs-target="#tab1-collapse_' . $permitID_row . '" aria-expanded="false" aria-controls="tab1-collapse_' . $permitID_row . '"><span class="bg-dark text-white p-2 rounded">' . $permitID_row . '</span> &nbsp;&nbsp;&nbsp;&nbsp; ';
                                // Print all results after the loop
                                foreach ($printableTest as $printable) {
                                    echo $printable;
                                }
                                echo '</button>
                        </h2>
                <div id="tab1-collapse_' . $permitID_row . '" class="accordion-collapse collapse" aria-labelledby="tab1-heading_' . $permitID_row . '" data-bs-parent="#accordion_' . $permitID_row . '">
                    <div class="accordion-body bg-black">';
                                echo '<div class="container px-1 px-md-0 py-1 mx-auto">
                <div class="card bg-black text-white">
                    ...
                    ';
                                ...
                                $specific_permit_id = $permitID_row;
                                ...
                                try {
                                    ...
                                    // فرض کنید که contractor_name از session یا روش دیگری بازیابی می‌شود
                                    // session_start();
                                    $contractor_name = $_SESSION['username'];

                                    // بازیابی نام پیمانکار بر اساس username
                                    $sql_get_fullname = "SELECT DISTINCT fullname FROM users WHERE username = :contractor_name";
                                    $stmt_get_fullname = $connection->prepare($sql_get_fullname);
                                    $stmt_get_fullname->bindParam(':contractor_name', $contractor_name, PDO::PARAM_STR);
                                    $stmt_get_fullname->execute();
                                    $fullname_result = $stmt_get_fullname->fetch(PDO::FETCH_ASSOC);
                                    $contractor_fullname = $fullname_result['fullname'];

                                    // بازیابی پروژه‌ها بر اساس contractor_name
                                    $sql_project = "SELECT DISTINCT * FROM project WHERE contractor_name = :contractor_fullname";
                                    $stmt_project = $connection->prepare($sql_project);
                                    $stmt_project->bindParam(':contractor_fullname', $contractor_fullname, PDO::PARAM_STR);
                                    $stmt_project->execute();
                                    $projects = $stmt_project->fetchAll(PDO::FETCH_ASSOC);

                                    foreach ($projects as $project) {
                                        $projectID = $project['id'];
                                        $contractorName = $project['contractor_name'];

                                        // SQL query with parameter for permit_id
                                        $sql1 = "
                                                    SELECT DISTINCT
                                                        pp.userID AS user_id,
                                                        u.username,
                                                        u.fullname,
                                                        pp.job_title AS job_title,
                                                        CASE 
                                                            WHEN ps.id IS NULL THEN 'itisnot'
                                                            ELSE 'itis'
                                                        END AS status_in_permit_status,
                                                        COALESCE(cia.actionStatus, 'noaction') AS action_status
                                                    FROM 
                                                        projectPersonnel pp
                                                    JOIN 
                                                        users u ON pp.userID = u.id
                                                    LEFT JOIN 
                                                        permit_status ps ON u.username = ps.user_id AND ps.permit_id = :permit_id
                                                    LEFT JOIN 
                                                        checklistItemsActions cia ON ps.permit_id = cia.permit_id
                                                    JOIN
                                                        project p ON pp.projectID = p.id
                                                    WHERE 
                                                        pp.projectID = :projectID AND pp.role != 'contractor' AND p.contractor_name = :contractor_name AND (cia.permit_id = :permit_id OR cia.permit_id IS NULL)
                                                ";
                                        ...
                                        // Bind parameters
                                        $stmt1->bindParam(':permit_id', $specific_permit_id, PDO::PARAM_STR);
                                        $stmt1->bindParam(':projectID', $projectID, PDO::PARAM_INT);
                                        $stmt1->bindParam(':contractor_name', $contractorName, PDO::PARAM_STR);
                                        ...
                                        // Start HTML output : permit_status
                                        echo '<div class="row d-flex justify-content-center">';
                                        echo '<div class="text-right"><p class="titleOfProcess text-black rounded w-25 p-3 mb-1 bg-white"> فرآیند پرمیت</p></div>';
                                        echo '<div class="bg-white col-12 border border-1 border-white rounded" style="padding: 10px 0px 20px; width: 98%;">';
                                        echo '<table class="table table-bordered table-striped text-center">';
                                        // echo '<thead class="thead-dark"><tr><th>نام و نام خانوادگی</th><th>عنوان شغلی</th><th>وضعیت فرآیند</th><th>وضعیت بازدید</th><th>تاریخ و ساعت آخرین بازدید</th></tr></thead>';
                                        echo '<thead class="thead-dark"><tr><th>نام و نام خانوادگی</th><th>عنوان شغلی</th><th>وضعیت فرآیند</th></tr></thead>';
                                        echo '<tbody>';

                                        foreach ($result1 as $row1) {

                                            echo '<tr>';
                                            echo '<td>' . htmlspecialchars($row1['fullname']) . '</td>';
                                            echo '<td>' . htmlspecialchars($row1['job_title']) . '</td>';
                                            if ($row1['status_in_permit_status'] == 'itis') {
                                                echo '<td class="bg-success text-dark ">انجام شده</td>';
                                            } else {
                                                echo '<td class="text-danger">انجام نشده</td>';
                                            }
                                        ...
                                        // Start HTML output : checklistitemsactions
                                        echo '<div class="row d-flex justify-content-center pt-5">';
                                        echo '<div class="text-right"><p class="titleOfProcess text-black rounded w-25 p-3 mb-1 bg-white"> فرآیند چک لیست</p></div>';
                                        echo '<div class="bg-white col-12 border border-1 border-white rounded" style="padding: 10px 0px 20px; width: 98%;">';
                                        echo '<table class="table table-bordered table-striped text-center">';
                                        // echo '<thead class="thead-dark"><tr><th>نام و نام خانوادگی</th><th>عنوان شغلی</th><th>وضعیت فرآیند</th><th>وضعیت بازدید</th><th>تاریخ و ساعت آخرین بازدید</th></tr></thead>';
                                        echo '<thead class="thead-dark"><tr><th>نام و نام خانوادگی</th><th>عنوان شغلی</th><th>وضعیت فرآیند</th></tr></thead>';
                                        ...
                                        foreach ($result1 as $row2) {
                                            echo '<tr>';
                                            echo '<td>' . htmlspecialchars($row2['fullname']) . '</td>';
                                            echo '<td>' . htmlspecialchars($row2['job_title']) . '</td>';
                                            if ($row2['action_status'] == 'noaction') {
                                                echo '<td class="text-danger">انجام نشده</td>';
                                            } else {
                                                echo '<td class="bg-success text-white">انجام شده</td>';
                                            }
                                            echo '</tr>';
                                        }
                                ...
                                $arrayDate = explode(' ', $row['date']);
                                list($yearDate, $monthDate, $dayDate) = explode('-', $arrayDate[0]);
                                list($hourDate, $minuteDate, $secondDate) = explode(':', $array[1]);
                                $timestampDate = mktime($hourDate, $minuteDate, $secondDate, $monthDate, $dayDate, $yearDate);
                                $jalaliDateDate = jdate("Y/m/d", $timestampDate);
            ...
            <div class="container text-white">
                <div class="row pb-3">
                    <div class="col-6 pt-3">نام پروژه: ' . $row['project_name'] . '</div>
                    <div class="col-6 pt-3">شماره قرارداد: ' . $row['contract_number'] . '</div>
                    <div class="col-6 pt-3">نام پیمانکار پروژه: ' . $row['contractor_name'] . '</div>
                    <div class="col-6 pt-3">تاریخ امضا قرارداد: ' . $jalaliDateDate . '</div>
                </div>
            </div>
        </div>
    </div>
                            ...
                            $_SESSION['registered_permitID'] = $permitID_row;
    ...
    color: white;
    position: relative;
    top: 85px;'>در حال بازسازی می باشد</td></tr>";
                    }
                    ...
                    //             users u ON pp.userID = u.id
                    //         LEFT JOIN 
                    //             permit_status ps ON u.username = ps.user_id
                    //         WHERE 
                    //             pp.projectID = 1 and pp.role != 'contractor'
                    //     ";
                    ...
                    //     // Start HTML output
                    //     echo '<div class="row d-flex justify-content-center">';
                    //         echo '<div class="text-right"><p class="titleOfProcess text-black rounded w-25 p-3 bg-white"> فرآیند پرمیت</p></div>';
                    //             echo '<div class="col-12 border border-1 border-white rounded" style="padding: 10px 0px 20px;width: 98%;">';
                    //                 echo '<ul id="progressbar" class="text-center">';

                    //     // Output each user's status in permit_status
                    //     foreach ($result as $row) {
                    //         if ($row['status_in_permit_status'] == "itis") {
                    //                 echo '<li class="active step0 text-white fs-3" style="padding: 5px 0px 5px 15px; background-color: white; color: black !important; border-radius: 10px; margin: 7px 30px 0px 0px;">' . htmlspecialchars($row['fullname']) . ' </li>';
                    //         } else {
                    //                 echo '<li class="step0 text-white fs-3" style="padding: 5px 0px 5px 15px; background-color: white; color: black !important; border-radius: 10px; margin: 7px 30px 0px 0px;">' . htmlspecialchars($row['fullname']) . ' </li>';
                    //         }
                    //     }
