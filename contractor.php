                    ...
                    <div class="row">
                        <div class="px-3">
                            <div class="row">
                                ...
                                // Function to get checklist counts
                                function getPermitCount($conn, $projectID, $status = NULL)
                                {
                                    $status = isset($status) ? $status : ''; // Default to empty string if $status is NULL
                                    ...
                                    if ($status != '') {
                                        ...
                                        WHERE
                                            ...
                                            AND permits.contractor_name = (
                                                SELECT contractor_name FROM project WHERE id = $projectID
                                            )
                                            AND permits.isDeleted = 0 
                                            AND permits.status = '$status'
                                    ";
                                    } else {
                                        $query = "
                                        SELECT
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        $count = $row['permitCount'];
                                    }
                                    return $count;
                                }

                                // Get counts for each type of permit
                                $allPermits = getPermitCount($conn, $projectID, '');
                                $newPermits = getPermitCount($conn, $projectID, 'issued');
                                $acceptedPermits = getPermitCount($conn, $projectID, 'Accept');
                                $rejectedPermits = getPermitCount($conn, $projectID, 'Reject');
                                ...
                                <div class="col-sm-12">
                                    <div class="card p-0">
                                        <div class="card-body">
                                            <h3 class="card-title">پرمیت ها</h3>
                                            <p class="card-text">برای نمایش همه پرمیت ها، بر روی لینک زیر کلیک کنید.</p>
                                            <div class="container">
                                                <div class="row position-relative">
                                                    <!-- All Permits -->
                                                    <div class="col">
                                                        <a href="./permits.php" class="btn btn-dark btn-sm rounded-3 mt-5 mb-0 position-relative">
                                                            همه پرمیت ها
                                                            <span class='badge bg-dark border border-3 border-dark opacity-50 text-white rounded-full'
                                                                style='position: absolute; top: -20px; right: -20px; font-size: 1rem;'><?= $allPermits ?></span>
                                                        </a>
                                                    </div>
                                                    <!-- New Permits -->
                                                    <div class="col">
                                                        <a href="./issuedContractorPermits.php" class="btn btn-success btn-sm rounded-3 mt-5 mb-0 position-relative">
                                                            پرمیت جدید
                                                            <span class='badge bg-dark opacity-50 border border-3 border-dark text-white rounded-full'
                                                                style='position: absolute; top: -20px; right: -20px; font-size: 1rem;'><?= $newPermits ?></span>
                                                        </a>
                                                    </div>
                                                    <!-- Accepted Permits -->
                                                    <div class="col">
                                                        <a href="./acceptedContractorPermits.php" class="btn btn-primary btn-sm rounded-3 mt-5 mb-0 position-relative">
                                                            پرمیت انجام شده
                                                            <span class='badge border border-3 border-dark bg-dark opacity-50 text-white rounded-s-full'
                                                                style='position: absolute; top: -20px; right: -20px; font-size: 1rem;'><?= $acceptedPermits ?></span>
                                                        </a>
                                                    </div>
                                                    <!-- Rejected Permits -->
                                                    <div class="col">
                                                        <a href="./rejectedContractorPermits.php" class="btn btn-danger btn-sm rounded-3 mt-5 mb-0 position-relative">
                                                            پرمیت ریجکت شده
                                                            <span class='badge border border-3 border-dark text-white bg-dark opacity-50 rounded-full'
                                                                style='position: absolute; top: -20px; right: -20px; font-size: 1rem;'><?= $rejectedPermits ?></span>
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                function getChecklistCount($conn, $projectID, $status = NULL)
                                {
                                    // Default status to NULL if not provided
                                    $status = isset($status) ? $status : NULL;

                                    // Base query (without status condition)
                                    $query = "
        SELECT
    COUNT(*) AS permitCount
FROM
...
WHERE
    ...
    ";
                                    ...
                                    // Add status condition if a valid status is provided
                                    if ($status !== NULL) {
                                        $query .= " AND checklist.status = ?";
                                    }
                                    ...
                                    if ($status !== NULL) {
                                        // 'iii' for projectID, projectID, and status
                                        $stmt->bind_param("iis", $projectID, $projectID, $status);
                                    } else {
                                        // 'ii' for projectID and projectID only
                                ...
                                // Get counts for each type of checklist
                                $allChecklists = getChecklistCount($conn, $projectID, NULL); // All checklists (no status filter)
                                $newChecklists = getChecklistCount($conn, $projectID, 'issued');
                                $acceptedChecklists = getChecklistCount($conn, $projectID, 'Accept');
                                $rejectedChecklists = getChecklistCount($conn, $projectID, 'Reject');
                                ...
                                <div class="col-sm-12">
                                    <div class="card p-0">
                                        <div class="card-body">
                                            <h3 class="card-title">چک لیست ها</h3>
                                            <p class="card-text">برای نمایش چک لیست ها، بر روی لینک زیر کلیک کنید.</p>
                                            <div class="row position-relative">
                                                <div class="col">
                                                    <a href="./checklists.php" class="btn btn-dark btn-sm rounded-3 mt-5 mb-0 position-relative">
                                                        همه چک لیست ها
                                                        <span class='badge bg-dark border border-3 border-dark opacity-50 text-white rounded-full'
                                                            style='position: absolute; top: -20px; right: -20px; font-size: 1rem;'><?php echo $allChecklists; ?></span>
                                                    </a>
                                                </div>
                                                <div class="col">
                                                    <a href="./issuedContractorChecklists.php" class="btn btn-success btn-sm rounded-3 mt-5 mb-0 position-relative">
                                                        چک لیست جدید
                                                        <span class='badge bg-dark opacity-50 border border-3 border-dark text-white rounded-full'
                                                            style='position: absolute; top: -20px; right: -20px; font-size: 1rem;'><?php echo $newChecklists; ?></span>
                                                    </a>
                                                </div>
                                                <div class="col">
                                                    <a href="./acceptedContractorChecklists.php" class="btn btn-primary btn-sm rounded-3 mt-5 mb-0 position-relative">
                                                        چک لیست انجام شده
                                                        <span class='badge border border-3 border-dark bg-dark opacity-50 text-white rounded-s-full'
                                                            style='position: absolute; top: -20px; right: -20px; font-size: 1rem;'><?php echo $acceptedChecklists; ?></span>
                                                    </a>
                                                </div>
                                                <div class="col">
                                                    <a href="./rejectedContractorChecklists.php" class="btn btn-danger btn-sm rounded-3 mt-5 mb-0 position-relative">
                                                        چک لیست ریجکت شده
                                                        <span class='badge border border-3 border-dark text-white bg-dark opacity-50 rounded-full'
                                                            style='position: absolute; top: -20px; right: -20px; font-size: 1rem;'><?php echo $rejectedChecklists; ?></span>
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
