...
// Title of Page:
$title = "Quality Assurance | Checklist Process";
...
-->
            ...
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
                    ...
                    <div class="container-xxl d-flex align-items-center justify-content-between" id="kt_header_container">
                        ...
                        <div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-2 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
                            ...
                            <ul class="breadcrumb fw-bold fs-base my-1">
                                ...
                                <li class="breadcrumb-item p-5 btn btn-outline-white rounded bg-white text-dark">فرآیند چک لیست ( گام اول )</li>
                                <!-- Left Side -->
                                <li class="breadcrumb-item p-5 btn btn-outline-white rounded bg-white text-dark text-left" style="position: absolute;left: 80px;" id="backPage"> صفحه قبل</li>
                                <script>
                ...
                // Retrieve permit ID from URL parameter
                $permit_id = $_GET['prmQWERTY'] ?? null;
                // Fetch permit details from the database based on the permit ID
                if ($permit_id) {
                    $query = "SELECT * FROM `permits` WHERE `permit_id` = ?";
                    ...
                    // use discipline id and get discipline name :
                    $stmt1GetDisciplineName = "SELECT * FROM `disciplines` WHERE `id` = ?";
                    $stmt1GetDisciplineName = $conn->prepare($stmt1GetDisciplineName);
                    $stmt1GetDisciplineName->bind_param("s", $permit['discipline']);
                    $stmt1GetDisciplineName->execute();
                    $stmt1GetDisciplineNameResult = $stmt1GetDisciplineName->get_result();
                    if ($stmt1GetDisciplineNameResult->num_rows > 0) {
                        $getDisciplineName = $stmt1GetDisciplineNameResult->fetch_assoc();
                    }
                }
                ?>
                <form action="checklistDone.php?prmQWERTY=<?php echo urlencode($permit_id); ?>" method="POST" class="form px-15" enctype="multipart/form-data">
                    <fieldset>
                        ...
                        <div class="container">
                            <div class="row">
                                ...
                                <div class="col-4 fv-row mb-10">
                                    <label class="d-flex align-items-center fs-5 fw-bold mb-2" class="form-label">
                                        <span class="required text-white">نام پروژه</span>
                                    </label>
                                    <input type="text" name="projectname" class="form-control text-center" value="<?= $permit['project_name'] ?? '' ?>" readonly>
                                </div>
                                <!-- Contractor Name (Read-only) -->
                                <div class="col-4">
                                    <label for="contractor_name" class="form-label text-white">نام پیمانکار</label>
                                    <input type="text" name="contractor_name" class="form-control text-center" value="<?= $permit['contractor_name'] ?? '' ?>" readonly>
                                </div>
                                <!-- Contract Number (Read-only) -->
                                <div class="col-4">
                                    <label for="contract_number" class="form-label text-white">شماره قرارداد</label>
                                    <input type="text" name="contract_number" class="form-control text-center" value="<?= $permit['contract_number'] ?? '' ?>" readonly>
                                </div>
                            </div>
                        </div>
                    </fieldset>
                    <fieldset>
                        <legend class="mb-10 text-white">دیسیپلین / دیسیپلین ها</legend>
                        <div class="container">
                            <div class="row">
                                ...
                                <div class="col-4 fv-row mb-10">
                                    ...
                                    <input type="text" name="discipline_group" class="form-control text-center" value="<?= $permit['discipline_group'] ?? '' ?>" readonly>
                                </div>
                                <!-- Discipline (Read-only) -->
                                <div class="col-4 fv-row mb-10">
                                    <label class="d-flex align-items-center fs-5 fw-bold mb-2">
                                        <span class="required text-white">دیسیپلین</span>
                                    </label>
                                    <input type="text" name="discipline" class="form-control text-center" value="<?= $permit['discipline'] ?? '' ?>" readonly>
                                </div>
                                <!-- Permit ID (Read-only) -->
                                <div class="col-4">
                                    <label for="permit_id" class="form-label text-white">شماره پرمیت متناظر</label>
                                    <input type="text" name="permit_id" class="form-control text-center" value="<?= $permit_id ?>" readonly>
                                </div>
                            </div>
                        </div>
                    </fieldset>
                    <fieldset>
                        <legend class="mb-10 text-white">سایر اطلاعات فرآیند چک لیست</legend>
                        <div class="container">
                            <div class="row">
                                <!-- Crew Input (Read-only) -->
                                <div class="col-4">
                                    <label for="crew" class="form-label text-white">پیمانکار</label>
                                    <input type="text" name="crew" class="form-control" value="<?= $permit['crew'] ?? '' ?>" readonly>
                                </div>
                                <!-- Axis Input (Read-only) -->
                                <div class="col-4">
                                    <label for="axis" class="form-label text-white">محور</label>
                                    <input type="text" name="axis" class="form-control" value="<?= $permit['axis'] ?? '' ?>" readonly>
                                </div>
                                <!-- Floor Input (Read-only) -->
                                <div class="col-4">
                                    <label for="floor" class="form-label text-white">طبقه</label>
                                    <input type="text" name="floor" class="form-control" value="<?= $permit['floor'] ?? '' ?>" readonly>
                                </div>
                                <!-- Block Input (Read-only) -->
                                <div class="col-4 mt-5">
                                    <label for="block" class="form-label text-white">بلوک</label>
                                    <input type="text" name="block" class="form-control" value="<?= $permit['block'] ?? '' ?>" readonly>
                                </div>
                                <!-- Zone Input (Read-only) -->
                                <!-- <div data-title="it is active field" class="col-8 mt-5 backgroundOfFieldUpload">
                                </div> -->
                                <div class="col-4 mt-5">
                                    <label for="zone" class="form-label text-white">زون</label>
                                    <input type="text" name="zone" class="form-control" value="<?= $permit['zone'] ?? '' ?>" readonly>
                                </div>
                                <!-- Operation Description Input (Read-only) -->
                                <div class="col-12 mb-5 mt-5">
                                    <label class="form-label text-white" for="operation_description">توضیحات عملیات</label>
                                    <textarea name="operation_description" class="form-control" rows="4" readonly><?= $permit['operation_description'] ?? '' ?></textarea>
                                </div>
                                <!-- Hidden input fields for data insertion -->
                                <input type="hidden" name="items[]" value="Checklist Item 1">
                                <input type="hidden" name="items[]" value="Checklist Item 2">
                                <!-- Add more hidden input fields for each checklist item -->
...
<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">بستن</button>
                    ...
                    <div class="container">
                        <div class="row">
                            ...
                            <div class="col-3">&nbsp;</div>
                            <div class="col-6">
                                <button type="submit" class="col-6 btn btn-white text-black form-control">بررسی آیتم های دیسیپلین <strong class="text-warning"> <?= $permit['discipline'] ?? '' ?></strong></button>
                            </div>
                        </div>
                    </div>
                </form>
