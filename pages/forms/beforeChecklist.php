...
// Title of Page:
$title = "Quality Assurance | Checklist Creation";
error_reporting(0);
...
-->
            ...
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
                <!--begin::Header-->
                <div id="kt_header" class="header" data-kt-sticky="true" data-kt-sticky-name="header" data-kt-sticky-offset="{default: '200px', lg: '300px'}" style="direction:rtl;">
                    <!--begin::Container-->
                    <div class="container-fluid d-flex align-items-center justify-content-between" id="kt_header_container">
                        ...
                        <div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-2 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
                            <!--begin::Heading-->
                            <!-- <h1 class="text-dark fw-bolder my-0 fs-2">فرآیند چک لیست جدید</h1> -->
                            <!--end::Heading-->
                            <!--begin::Breadcrumb-->
                            <ul class="breadcrumb fw-bold fs-base my-1">
                                ...
                                <li class="breadcrumb-item p-5 btn btn-outline-white bg-black text-white rounded" style="border: 1px solid white">چک لیست</li>
                                <!-- Left Side -->
                                <li class="breadcrumb-item p-5 btn btn-outline-white rounded bg-white text-dark text-left" style="position: absolute;left: 30px;" id="backPage"> صفحه قبل</li>
                                <script>
                ...
                <!--begin::Content-->
                <!--begin::Table head-->
                <div class="px-10" style="direction:rtl;">
                    <table class="table px-10">
                        <thead>
                            <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                                <th class="min-w-15px text-start">نام پروژه</th>
                                <th class="min-w-15px text-start">نام پیمانکار</th>
                                <th class="text-start min-w-50px text-start">شماره پرمیت</th>
                                <th class="min-w-15px text-start">وضعیت چک لیست </th>
                                <th class="min-w-15px text-start">تاریخ و زمان ایجاد فرم چک لیست</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            ...
                            // Check if project_id is set in session
                            if (!isset($_SESSION['project_id'])) {
                                die("Project ID is not set in the session.");
                            }
                            ...
                            $query = 'SELECT * FROM `permits` WHERE `project_name` = ( SELECT `project_name` FROM `project` WHERE `id` = ? ) AND `contractor_name` = (SELECT `contractor_name` FROM `project` WHERE `id` = ? ) ORDER BY permits.dateTimeCreated DESC';
                            ...
                            // Bind the project ID to both placeholders
                            $stmt->bind_param("ii", $project_id, $project_id);
                            $stmt->execute();
                            $result = $stmt->get_result();

                            // Display permits in table rows
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    $permitID = $row['permit_id'];

                                    // Check if the permit is processed in the status table
                                    $statusQuery = "SELECT * FROM `status` WHERE `proccessID` = ? AND `status` = 'Accept' AND `type` = 'Permit'";
                                    $statusStmt = $conn->prepare($statusQuery);
                                    $statusStmt->bind_param("s", $permitID);
                                    $statusStmt->execute();
                                    $statusResult = $statusStmt->get_result();
                                    $isProcessed = $statusResult->num_rows > 0;

                                    // Check if there is a record in the checklist table
                                    $checklistQuery = "SELECT * FROM `checklist` WHERE `permit_id` = ? AND `projectname` = ( SELECT `project_name` FROM `project` WHERE `id` = ? ) AND `contractor_name` = ( SELECT `contractor_name` FROM `project` WHERE `id` = ? ) ORDER BY checklist.date_created DESC";
                                    $checklistStmt = $conn->prepare($checklistQuery);
                                    $checklistStmt->bind_param("sii", $permitID, $project_id, $project_id);
                                    $checklistStmt->execute();
                                    $checklistResult = $checklistStmt->get_result();

                                    // Handle processed permits and checklist records
                                    if ($isProcessed) {
                                        if ($checklistResult->num_rows > 0) {
                                            // Permit is processed and there is a record in checklist table
                                            while ($rowChecklist = $checklistResult->fetch_assoc()) {
                                                $checklistDate = $rowChecklist['date_created'];
                                                $timestamp = strtotime($checklistDate);

                                                // Display issued status
                                                echo '<tr>';
                                                echo "<td>{$rowChecklist['projectname']}</td>";
                                                echo "<td>{$rowChecklist['contractor_name']}</td>";
                                                echo "<td><strong>{$row['permit_id']}</strong></td>";
                                                if ($rowChecklist['isDeleted'] == false) {
                                                    if ($rowChecklist['status'] == "issued") {
                                                        echo "<td><p class='statusSize bg-warning text-white p-2 rounded text-center'>ایجاد شده</p></td>";
                                                    } else {
                                                        echo "<td><p class='statusSize bg-129045 text-white p-2 rounded text-center'>انجام شده</p></td>";
                                                    }
                                                } else {
                                                    echo "<td><p class='bg-danger text-white p-2 rounded text-center text-xs' style='min-width: 150px;'>حذف شده توسط ادمین</p></td>";
                                                }

                                                echo "<td>" . jdate("تاریخ:Y/m/d زمان:H:i:s", $timestamp) . "</td>";
                                                echo '</tr>';
                                            }
                                        } else {
                                            // Permit is processed but no record in checklist table
                                            echo "<tr class='clickable-row' data-href='checklist.php?prmQWERTY={$row['permit_id']}'>";
                                            echo "<td>{$row['project_name']}</td>";
                                            echo "<td>{$row['contractor_name']}</td>";
                                            echo "<td><strong><a href='checklist.php?prmQWERTY={$row['permit_id']}' class='btn btn-dark text-white btn-sm'>{$row['permit_id']}</a></strong></td>";
                                            echo "<td><p class='statusSize bg-c122334 text-white p-2 rounded text-center'>ایجاد فرآیند</p></td>";
                                            echo "<td>" . jdate("تاریخ:Y/m/d زمان:H:i:s", strtotime($currentDateTime)) . "</td>";
                                            echo "</tr>";
                                        }
                                    }
                                }
                                ...
                                // No permits found
                                echo "<tr><td colspan='4'>هیچ فرآیند چک لیستی برای پروژه " . $_SESSION['project_name'] . " یافت نشد</td></tr>";
                            }
