...
$title = "Quality Assurance | Owner Dashboard | Every Personnel's Process Flow Status";
require_once('../../datetime/jdf.php');
error_reporting(0);
?>
<!DOCTYPE html>
<!--
...
<body id="kt_body" style="background-image: url();background-color: #000;" class="header-fixed header-tablet-and-mobile-fixed aside-fixed aside-secondary-enabled">
    <!--begin::Main-->
    <!--begin::Root-->
        ...
        <div class="page d-flex flex-row flex-column-fluid">
            ...
            <div id="kt_aside" style="background-color: black;" class="aside aside-extended flex-row-reverse " data-kt-drawer="true" data-kt-drawer-name="aside" data-kt-drawer-activate="{default: true, lg: false}" data-kt-drawer-overlay="true" data-kt-drawer-width="auto" data-kt-drawer-direction="start" data-kt-drawer-toggle="#kt_aside_toggle">
                ...
                <div class="aside-secondary d-flex flex-row-fluid">
                    <!--begin::Workspace-->
                    <div class="aside-workspace my-5 p-3 no-hover" id="kt_aside_wordspace">
                                                    ...
                                                    <form data-kt-search-element="form" class="w-100 position-relative mb-5 mb-lg-0" autocomplete="off">
                                                        ...
                                                        <span class="text-center">
                                                            ...
                                                            if ($rowFullnameResult) {
                                                                ...
                                                                $conclusion = $role == "owner" ? " کارفرما" : "پیمانکار";
                                                                echo $full_name . " ( " . $jobTitle . " - " . $conclusion . " ) ";
                                                                echo "</div><br/>";
                                                            } else {
...
-->
            ...
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
                ...
                <div id="kt_header" class="header" data-kt-sticky="true" data-kt-sticky-name="header" data-kt-sticky-offset="{default: '200px', lg: '300px'}">
                    ...
                    <div class="container-fluid d-flex align-items-center justify-content-between" id="kt_header_container">
                        ...
                        <div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-2 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
                            ...
                            <ul class="breadcrumb fw-bold fs-base my-1">
                                <li class="breadcrumb-item text-muted">
                                    <a href="/contractor.php" class="p-5 btn btn-white text-dark rounded with-hover">صفحه نخست</a>
                                </li>
                                <li class="breadcrumb-item p-5 btn rounded bg-black text-white" style="border: 1px solid white">
                                    کارتابل وضعیت آنی فرآیند
                                    <?php
                                    echo "<span style='font-size: 17px;margin-right: 10px;'>" . $_GET['itemID'] . "</span>";
                                    ?>
                                </li>
                        ...
                        <div class="d-flex d-lg-none align-items-center ms-n2 me-2">
                            ...
                            <div class="btn btn-icon btn-active-icon-primary mobile_view" id="kt_aside_toggle">
                            ...
                            <a href="/index.php" class="d-flex align-items-center">
                                <img alt="Logo" src="../../assets/media/logos/mika-main.png" class="h-30px thisview" />
                            </a>
                            <!--end::Logo-->
                        </div>
                        <!--end::Wrapper-->
                ...
                <!--end::Header-->
                <!--begin::Content-->
                <!-- It is about body content of the project by Arash Ataei ... -->
                <!--begin::Content-->
                <div class="mobile_view text-left col-sm-4" style="z-index:999999999999;">
                    <a href="../../owner.php" class="breadcrumb-item p-5 btn btn-sm btn-outline-white rounded bg-white text-dark text-left">
                        صفحه قبل
                    </a>
                </div>
                <?php
                ...
                $specific_item_id = $_GET['itemID'] ?? null;
                if (!$specific_item_id) {
                    die("Item ID is required");
                }

                // Validate itemID format
                if (!preg_match('/^[\w-]+$/', $specific_item_id)) {
                    die("Invalid Item ID format");
                }
                ...
                // Fetch the checklist or permit ID
                $permit_id = null;
                $item_type = 'permit'; // Default to 'permit'

                $sql_checklist = "SELECT checklist_id, permit_id FROM checklist WHERE checklist_id = ?";
                $stmt_checklist = $conn->prepare($sql_checklist);
                $stmt_checklist->bind_param("s", $specific_item_id);
                $stmt_checklist->execute();
                $result_checklist = $stmt_checklist->get_result();

                if ($result_checklist->num_rows > 0) {
                    // The item is a checklist
                    $row_checklist = $result_checklist->fetch_assoc();
                    $item_type = 'checklist';
                    $permit_id = $row_checklist['permit_id'];
                } else {
                    // Item is a permit
                    $permit_id = $specific_item_id;
                }
                $stmt_checklist->close();

                // Fetch the user's projectID
                $sql_user_project = "SELECT projectID FROM projectPersonnel 
INNER JOIN users ON users.id = projectPersonnel.userID 
WHERE users.username = ?";
                $stmt_user_project = $conn->prepare($sql_user_project);
                $stmt_user_project->bind_param("s", $current_username);
                $stmt_user_project->execute();
                $result_user_project = $stmt_user_project->get_result();
                $user_project = $result_user_project->fetch_assoc();
                $projectID = $user_project['projectID'] ?? null;
                $stmt_user_project->close();

                if (!$projectID) {
                    die("User's projectID not found");
                }

                // Fetch project personnel
                $sql_personnel = "SELECT userID, users.fullname, projectPersonnel.job_title, projectPersonnel.projectID 
FROM projectPersonnel 
INNER JOIN users ON users.id = projectPersonnel.userID 
WHERE projectPersonnel.projectID = ? AND projectPersonnel.role != 'contractor'";
                $stmt_personnel = $conn->prepare($sql_personnel);
                $stmt_personnel->bind_param("i", $projectID);
                $stmt_personnel->execute();
                $result_personnel = $stmt_personnel->get_result();

                // Initialize array to store completion status
                $completion_status = [];

                while ($row_personnel = $result_personnel->fetch_assoc()) {
                    $userID = $row_personnel['userID'];
                    $fullname = $row_personnel['fullname'];
                    $job_title = $row_personnel['job_title'];
                    $submissionDateTime = 'ندارد'; // Default value if no date is found

                    // Check completion status
                    if ($_GET['type'] === 'checklist') {
                        $checklist_permit_id = str_replace("C", "P", $_GET['itemID']);
                        $sql_checklist_status = "SELECT COUNT(*) as mainCount, MAX(submitted_at) as submitted_at, user_name
                                  FROM checklistItemsActions
                                  WHERE permit_id = ? AND user_name = (SELECT username FROM users WHERE id = ?)
                                  GROUP BY user_name";
                        $stmt_checklist_status = $conn->prepare($sql_checklist_status);
                        $stmt_checklist_status->bind_param("si", $checklist_permit_id, $userID);
                        $stmt_checklist_status->execute();
                        $result_checklist_status = $stmt_checklist_status->get_result();
                        $row_checklist_status = $result_checklist_status->fetch_assoc();
                        $completed = ($row_checklist_status['mainCount'] > 0) ? 'تکمیل شده' : 'تکمیل نشده';
                        $submitted_at = $row_checklist_status['submitted_at'];
                        $submissionDateTime = !empty($submitted_at) ? jdate('H:i:s - Y/m/d', strtotime($submitted_at)) : 'ندارد';
                        $user_name = $row_checklist_status['user_name'];
                    } else {
                        $sql_permit_status = "SELECT COUNT(*) as count, MAX(dateTimeCreated) as dateTimeCreated, user_id
                              FROM permit_status 
                              WHERE permit_id = ? AND user_id = (SELECT username FROM users WHERE id = ?)
                              GROUP BY user_id";
                        $stmt_permit_status = $conn->prepare($sql_permit_status);
                        $stmt_permit_status->bind_param("si", $permit_id, $userID);
                        $stmt_permit_status->execute();
                        $result_permit_status = $stmt_permit_status->get_result();
                        $row_permit_status = $result_permit_status->fetch_assoc();
                        $completed = ($row_permit_status['count'] == 1) ? 'تکمیل شده' : 'تکمیل نشده';
                        $dateTimeCreated = $row_permit_status['dateTimeCreated'];
                        $submissionDateTime = !empty($dateTimeCreated) ? jdate('H:i:s - Y/m/d', strtotime($dateTimeCreated)) : 'ندارد';
                        $user_name = $row_permit_status['user_id'];
                    }
                    ...
                    $completion_status[] = [
                        ...
                        'submission_date' => htmlspecialchars($submissionDateTime),
                        'user_name' => htmlspecialchars($user_name)
                    ];
                }

                // Get the project name
                $sql_project_name = "SELECT project_name FROM project WHERE id = ?";
                $stmt_project_name = $conn->prepare($sql_project_name);
                $stmt_project_name->bind_param("i", $projectID);
                $stmt_project_name->execute();
                $result_project_name = $stmt_project_name->get_result();
                $row_project_name = $result_project_name->fetch_assoc();
                $project_name = $row_project_name['project_name'] ?? 'Unknown Project';

                // Display results in a table
                echo '<div class="container-fluid px-0 mt-10 text-center" DIR="rtl">';
                echo '<h2 class="text-center text-white">وضعیت تکمیل فرم <span class="text-warning">' . htmlspecialchars($permit_id) . '</span> برای اعضای پروژه <span class="text-warning">' . htmlspecialchars($project_name) . '</span></h2>';
                echo '<div class="bg-secondary p-5" style="border-radius: 20px;margin-top: 50px">';
                echo '<table class="table table-bordered text-center text-black" DIR="rtl">';
                echo '<thead class="thead-dark text-dark">';
                echo '<tr>';
                echo '<th>نام و نام خانوادگی</th>';
                echo '<th>عنوان شغلی</th>';
                echo '<th>وضعیت تکمیل</th>';
                echo '<th>زمان تکمیل فرم</th>';
                echo '<th>عملیات</th>';
                echo '</tr>';
                echo '</thead>';
                echo '<tbody>';

                foreach ($completion_status as $person) {
                    $bg_class = ($person['status'] == 'تکمیل شده') ? 'bg-success text-success' : 'bg-danger text-danger';
                    echo '<tr class="' . $bg_class . '">';
                    echo '<td>' . $person['fullname'] . '</td>';
                    echo '<td>' . $person['job_title'] . '</td>';
                    echo '<td>' . $person['status'] . '</td>';
                    echo '<td>' . $person['submission_date'] . '</td>';
                    if ($person['status'] === 'تکمیل شده') {
                        echo '<td><a class="btn btn-dark btn-sm w-100" href="view_form.php?username=' . urlencode($person['user_name']) . '&itemID=' . urlencode($specific_item_id) . '">مشاهده فرم تکمیل شده</a></td>';
                    } else {
                        echo '<td><span>فرم تکمیل نشده</span></td>';
                    }
                    echo '</tr>';
                }
