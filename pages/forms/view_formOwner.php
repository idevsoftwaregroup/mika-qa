...
session_start();
error_reporting(0);
// the current permit ID :
$currentPermitID = $_SESSION['theCurrentPermitIDThatHasBeenSet'];
// Error Handling
...
// Title of Page:
$title = "Quality Assurance |‌ نمایش اطلاعات فرم پرمیت";
...
-->
                                            ...
                                            <div class="m-0">
                                                ...
                                                <div class="mb-10">
                                                    ...
                                                    <br />
                                                    <!--begin::Item-->
                                                    <a href="<?php echo ($role === "owner" ? "/pages/forms/projectsOwner.php" : "/pages/forms/projects.php") ?>" class="custom-list d-flex align-items-center px-5 py-1">
                                                        <div class="d-flex flex-column flex-grow-1">
                                                            <!--begin::Link-->
            ...
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
                ...
                <div id="kt_header" class="header" data-kt-sticky="true" data-kt-sticky-name="header" data-kt-sticky-offset="{default: '200px', lg: '300px'}">
                    ...
                    <div class="container-fluid d-flex align-items-center justify-content-between" id="kt_header_container">
                        ...
                        <div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-2 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
                            <!--begin::Heading-->
                            <!-- <h1 class="text-dark fw-bolder my-0 fs-2">تایید نهایی فرآیند پرمیت</h1> -->
                            <!--end::Heading-->
                            <!--begin::Breadcrumb-->
                            <ul class="breadcrumb fw-bold fs-base my-1">
                                ...
                                <li class="breadcrumb-item p-5 btn rounded bg-white text-dark">
                                    نمایش اطلاعات فرم &nbsp;
                ...
                $username = $_GET['username'] ?? 'نامشخص';
                $itemID = $_GET['itemID'] ?? 'نامشخص';
                $type = substr($itemID, 4, 1) === "P" ? "Permit" : "Checklist";


                // Get the name of the current user
$cUserQuery = "SELECT fullname FROM users WHERE username = ?";
$stmtCurrentUser = $conn->prepare($cUserQuery);
$stmtCurrentUser->bind_param("s", $_GET['username']); // 'bind_param' instead of 'bind_params'
$stmtCurrentUser->execute();
$stmtResultCurrentUser = $stmtCurrentUser->get_result();
$user = $stmtResultCurrentUser->fetch_assoc();
$fullName = $user['fullname'];


                // تابع برای دیکد کردن JSON و نمایش به صورت لیست
                function decodeJson($json)
                {
                    $items = json_decode($json, true);
                    if (!$items) return "<li>نامشخص</li>";
                ...
                // متغیرهای برای نگهداری اطلاعات
                $projectInfo = "";
                $itemsList = "";
                $disciplineItem = ""; // متغیر برای ذخیره دیسیپلین

                // دریافت داده‌ها بر اساس نوع
                if ($type === "Permit") {
                    // دریافت اطلاعات پرمیت
                    $sqlProject = "SELECT * FROM permits WHERE permit_id = ?";
                    $stmtProject = $conn->prepare($sqlProject);
                    $stmtProject->bind_param("s", $itemID);
                    $stmtProject->execute();
                    $resultProject = $stmtProject->get_result();

                    if ($resultProject->num_rows > 0) {
                        $projectData = $resultProject->fetch_assoc();
                        $projectInfo = "<div class='d-flex justify-content-end mb-3'>";
                        $projectInfo .= "<button onclick='printProjectInfo()' class='btn btn-primary me-2'>چاپ</button>";
                        $projectInfo .= "<button onclick='saveAsWord()' class='btn btn-secondary'>ذخیره به عنوان Word</button>";
                        $projectInfo .= "</div>";
                        $projectInfo .= "<div id='project-content' class='card shadow-lg p-10'>";
                        $projectInfo .= "<h3 class='fs-2'>اطلاعات پروژه</h3>";
                        $projectInfo .= "<div class='p-10 fs-4'>";
                        $projectInfo .= "<p>نام پروژه: " . htmlspecialchars($projectData['project_name']) . "</p>";
                        $projectInfo .= "<p>شماره قرارداد: " . htmlspecialchars($projectData['contract_number']) . "</p>";
                        $projectInfo .=
                            "<p>شماره پرمیت: " . htmlspecialchars($projectData['permit_id']) . "</p>";
                        $projectInfo .=
                            "<p>پیش بینی مدت زمان اجرا: " . str_replace(['days', 'hours'], ['روز', 'ساعت'], htmlspecialchars($projectData['time_to_action'])) . "</p>";
                        $projectInfo .= "<p>مدت زمان اعتبار پرمیت: " . str_replace(['days', 'hours'], ['روز', 'ساعت'], htmlspecialchars($projectData['permit_credit'])) . "</p>";
                        $projectInfo .= "<p>نام پیمانکار: " . htmlspecialchars($projectData['contractor_name']) . "</p>";
                        $projectInfo .= "<p>توضیحات عملیات: " . htmlspecialchars($projectData['operation_description']) . "</p>";
                        // Status
                        $projectInfo .= "<p><strong>وضعیت:</strong> " .
                            (htmlspecialchars($projectData['status']) === "Accept" ? "<span class='badge bg-success text-white'>تایید شده</span>" : (htmlspecialchars($projectData['status']) === "Reject" ? "<span class='badge bg-danger text-white'>تایید نشده</span>" : (htmlspecialchars($projectData['status']) === "N/A" ? "<span class='badge bg-secondary text-white'>N/A</span>" : (htmlspecialchars($projectData['status']) === "in progress" ? "<span class='badge bg-info text-white'>در حال بررسی</span>" : (htmlspecialchars($projectData['status']) === "issued" ? "<span class='badge bg-warning text-white'>ایجاد شده</span>" :
                                "<span class='badge bg-dark text-white'>بدون وضعیت</span>"))))) . "</p>";
                                $projectInfo .= "<p class='p-3 bg-secondary border-2 ' style='border-style: dashed;border-radius: 19px;'>نام تکمیل کننده : ".$fullName."</p>";
                        $projectInfo .= "</div>";

                        // دریافت آیتم‌های پرمیت
                        $sqlItems = "SELECT items, discipline FROM permit_status WHERE permit_id = ? and user_id = ?";
                        $stmtItems = $conn->prepare($sqlItems);
                        $stmtItems->bind_param("ss", $itemID, $_GET['username']);
                        $stmtItems->execute();
                        $resultItems = $stmtItems->get_result();

                        if ($resultItems->num_rows > 0) {
                            while ($row = $resultItems->fetch_assoc()) {
                                $itemsList .= '<ul class=" p-4 mt-4 p-10">' . decodeJson($row['items']) . '</ul>';
                                $disciplineItem = htmlspecialchars($row['discipline']); // ذخیره دیسیپلین آخرین آیتم
                            }
                        } else {
                            $itemsList = '<p>هیچ آیتمی یافت نشد</p>';
                        }
                    } else {
                        $projectInfo = "<p>اطلاعات پرمیت یافت نشد.</p>";
                    }
                } elseif ($type === "Checklist") {
                    // دریافت اطلاعات چک لیست
                    $sqlProject = "SELECT * FROM checklist WHERE checklistID = ?";
                    $stmtProject = $conn->prepare($sqlProject);
                    $stmtProject->bind_param("s", $itemID);
                    $stmtProject->execute();
                    $resultProject = $stmtProject->get_result();
                    ...
                    if ($resultProject->num_rows > 0) {
                        ...
                        $projectInfo .= "<button onclick='saveAsWord()' class='btn btn-secondary'>ذخیره به عنوان Word</button>";
                        $projectInfo .= "</div>";
                        $projectInfo .= "<div id='project-content'>";
                        $projectInfo .= "<div class='container-fluid my-4 p-0'>";
                        $projectInfo .= "<h3 class='mb-4 fs-1'>اطلاعات پروژه</h3>";
                        $projectInfo .= "<div class='card p-5 shadow-lg fs-4'>";
                        $projectInfo .= "<p><strong>نام پروژه:</strong> " . htmlspecialchars($projectData['projectname']) . "</p>";
                        $projectInfo .= "<p><strong>شماره چک لیست:</strong> " . htmlspecialchars($projectData['checklistID']) . "</p>";
                        $projectInfo .= "<p><strong>نام پیمانکار:</strong> " . htmlspecialchars($projectData['contractor_name']) . "</p>";
                        $projectInfo .= "<p><strong>توضیحات:</strong> " . htmlspecialchars($projectData['operation_description']) . "</p>";
                        ...
                        // Date Created
                        $projectInfo .= "<p><strong>تاریخ ایجاد فرم چک لیست:</strong> " . jdate('h:i:s - Y/m/d', strtotime($projectData['date_created'])) . "</p>";
                        $projectInfo .= "<p class='p-3 bg-secondary border-2 ' style='border-style: dashed;border-radius: 19px;'>نام تکمیل کننده : ".$fullName."</p>";
                        $projectInfo .= "</div></div>";

                        // Checklist Items
                        $sqlItems = "SELECT item, discipline FROM checklistItemsActions WHERE checklistItemsActions.permit_id = ? AND checklistItemsActions.user_name = ?";
                        $stmtItems = $conn->prepare($sqlItems);
                        $stmtItems->bind_param("ss", str_replace("-C-", "-P-", $itemID), $username);
                        $stmtItems->execute();
                        $resultItems = $stmtItems->get_result();

                        if ($resultItems->num_rows > 0) {
                            $itemsList .= '<div class="card p-4 mt-4 shadow-lg"><h4 class="mb-3">آیتم‌های چک لیست</h4><ul class="list-group">';
                            while ($row = $resultItems->fetch_assoc()) {
                                $itemsList .= '<li class="list-group-item">' . htmlspecialchars($row['item']) . '</li>';
                                $disciplineItem = htmlspecialchars($row['discipline']); // Save last item's discipline
                            }
                            $itemsList .= '</ul></div></div>';
                        } else {
                            $itemsList = '<div class="alert alert-warning mt-4">هیچ آیتمی یافت نشد</div>';
                        }
                    } else {
                        $projectInfo = "<div class='alert alert-danger'>اطلاعات چک لیست یافت نشد.</div>";
                    }
                }

                // نمایش داده‌ها
                echo '<div class="container bg-white mr-20 my-15 p-20 w-75 h-auto" style="border-radius: 20px;" dir="RTL" >';
                echo $projectInfo;  // نمایش اطلاعات پروژه
                echo "<p>&nbsp;</p>";
                if ($type === "Checklist" && !empty($disciplineItem)) {
                    echo "<h2 class='mb-5'>لیست آیتم‌های چک لیست - " . $disciplineItem . "</h2>";
                } else {
                    echo "<h2 class='mb-5'>لیست آیتم‌های پرمیت - " . $disciplineItem . "</h2>";
                }
                echo $itemsList;
                echo '</div>';
                ...
                <script>
                    // Function to Print the Content of Page
                    function printProjectInfo() {
                        var printContent = document.getElementById('project-content').innerHTML;
                        var originalContent = document.body.innerHTML;

                        // Add RTL styling
                        document.body.innerHTML = `<div style="direction: rtl; text-align: right;">${printContent}</div>`;
                        window.print();
                        document.body.innerHTML = originalContent;

                        // Reload the page to restore any lost event listeners after replacing the document's HTML
                        window.location.reload();
                    }

                    // Function to save the project info as a Word document
                    function saveAsWord() {
                        var preHtml = "<html><head><meta charset='utf-8'><title>اطلاعات پروژه</title></head><body>";
                        var postHtml = "</body></html>";
                        var html = preHtml + document.getElementById('project-content').innerHTML + postHtml;
                ...
                <!-- استایل‌ها برای نمایش ریسپانسیو -->
                <style>
