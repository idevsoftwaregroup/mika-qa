...
// Title of Page:
$title = "Quality Assurance | Permit Items";
...
// Retrieve and sanitize permit ID from GET parameter
$permitIDHitter = $_GET['prmQWERTY'];
$permitIDHitter = $conn->real_escape_string($permitIDHitter); // Example of basic sanitization

// Get current date and time
$currentDate = date('Y-m-d'); // Format: YYYY-MM-DD
$currentTime = date('H:i:s'); // Format: HH:MM:SS

// Function to log seen items
function logSeen($conn, $item_id, $user_id, $item_type)
{
    // Prepare and bind a select statement to check for existence
    $check_stmt = $conn->prepare("SELECT id FROM seen_log WHERE item_id = ? AND user_id = ? AND item_type = ?");
    $check_stmt->bind_param("sss", $item_id, $user_id, $item_type);
    $check_stmt->execute();
    ...
    if ($check_stmt->num_rows == 0) {
        ...
        $insert_stmt = $conn->prepare("INSERT INTO seen_log (item_id, user_id, item_type, seen_at) VALUES (?, ?, ?, NOW())");
        $insert_stmt->bind_param("sss", $item_id, $user_id, $item_type);
        ...
        // If the record exists, update the seen_at timestamp
        $update_stmt = $conn->prepare("UPDATE seen_log SET seen_at = NOW() WHERE item_id = ? AND user_id = ? AND item_type = ?");
        $update_stmt->bind_param("sss", $item_id, $user_id, $item_type);
...
// example insertion of the system :
$item_type = 'Permit';
logSeen($conn, $_GET['prmQWERTY'], $_SESSION['username'], $item_type);
?>
<!DOCTYPE html>
<html lang="fa">
<!--begin::Head-->
...
<head>
    ...
    <!--begin date picker   -->
    <link rel="stylesheet" href="../../assets/js/jalaly-date-picker/jalalidatepicker.css">
    <!--end date picker   -->
</head>
<!--end::Head-->
<!--begin::Body-->
                        ...
                        <div class="d-flex h-100 flex-column">
                          ...
                          WHERE u.username = ?";
                                                            ...
                                                            if ($rowFullnameResult) {
                                                                ...
                                                                echo $full_name . " ( " . $jobTitle . " - " . ($role == "owner" ? " کارفرما" : "پیمانکار") . " ) ";
                                                                echo "</div><br/>";
                                                            } else {
...
-->
                                            ...
                                            <div class="m-0">
                                                ...
                                                <div class="mb-10">
                                                    <!--begin::Item-->
                                                    <a href="/owner.php" class="custom-list d-flex align-items-center px-5 py-1">
                                                        <!--begin::Symbol-->
                                                    ...
                                                    <br />
                                                    <!--begin::Item-->
                                                    <a href="/settingsOwner.php" class="custom-list d-flex align-items-center px-5 py-1">
                                                        <!--begin::Symbol-->
                                                        ...
                                                        <div class="d-flex flex-column flex-grow-1">
                                                            ...
                                                            <!--end::Title-->
                                                            <!--begin::Link-->
                                                            <span class="text-white fw-bold" style="border: 1px solid white;padding: 10px;border-radius: 5px;"> تنظیمات</span>
                                                            <!--end::Link-->
                                                        </div>
                                                        <!--begin::Description-->
                                                    </a>
                ...
                <!--end::Secondary-->
                <!--begin::Aside Toggle-->
                <button class="btn btn-sm btn-icon bg-body btn-color-gray-700 btn-active-primary position-absolute translate-middle start-100 start-50 bottom-0 shadow-sm d-none d-lg-flex" data-kt-toggle="true" data-kt-toggle-state="active" data-kt-toggle-target="body" data-kt-toggle-name="aside-minimize" style="margin-bottom: 1.35rem">
            ...
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
                ...
                <div id="kt_header" class="header" data-kt-sticky="true" data-kt-sticky-name="header" data-kt-sticky-offset="{default: '200px', lg: '300px'}">
                    ...
                    <div class="container-fluid d-flex align-items-center justify-content-between" id="kt_header_container">
                        ...
                        <div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-2 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
                            <!--begin::Heading-->
                            <!-- <h1 class="text-dark fw-bolder my-0 fs-2">آیتم های فرم ( فرآیند ) پرمیت</h1> -->
                            <!--end::Heading-->
                            <!--begin::Breadcrumb-->
                            <ul class="breadcrumb fw-bold fs-base my-1">
                                <li class="breadcrumb-item text-muted">
                                    <a href="/contractor.php" class="p-5 btn btn-outline-white rounded text-black bg-white">صفحه نخست</a>
                                </li>
                                <li class="breadcrumb-item p-5 btn btn-outline-white rounded bg-black text-white" style="border: 1px solid white"> فرآیند پرمیت</li>
                                <!-- Left Side -->
                                <li class="breadcrumb-item p-5 btn btn-outline-white rounded bg-white text-dark text-left" style="position: absolute;left: 37px;" id="backPage"> صفحه قبل</li>
                                <script>
                ...
                <!--end::Header-->
                <!--begin::Content-->
                <form method="post" class="container-fluid form px-5" action="permitItemsDone.php">
                    <div class="container-fluid">
                        <div class="row">
                            <div class="accordion-item mb-5 col-12 rounded-2">
                                <h2 class="accordion-header" id="tabmain-heading">
                                    <button class="accordion-button text-black" type="button" data-bs-toggle="collapse" data-bs-target="#tab1-collapse" aria-expanded="true" aria-controls="tab1-collapse">
                                        اطلاعات پروژه
                                    </button>
                                </h2>
                                <div id="tab1-collapse" class="accordion-collapse collapse show" aria-labelledby="tabmain-heading" data-bs-parent="#accordion">
                                    <div class="accordion-body">
                                        <!-- get the project information -->
                                        <fieldset>
                                            <legend class="mb-10 text-muted">اطلاعات پروژه</legend>
                                            <div class="container">
                                                <div class="row">
                                                    <!--begin::Project dropdown-->
                                                    <div class="col-4 fv-row mb-10">
                                                        <label class="d-flex align-items-center fs-5 fw-bold mb-2">
                                                            <span class="">نام پروژه</span>
                                                        </label>
                                                        <?php
                                                        // Database connection parameters
                                                        $host = "localhost";
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT project_name FROM project WHERE project.contract_number = (SELECT permits.contract_number FROM permits WHERE permits.permit_id = ?)";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        $stmt->execute();
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            ...
                                                            // Display project name in readonly input field
                                                            echo '<input type="text" name="project_name" id="project_name" value="' . $projectName . '" class="form-control text-center text-" readonly disabled="disabled" bg-secondary>';
                                                        } else {
                                                            echo "نام پروژه ثبت نشده است";
                                                        }
                                                        ?>
                                                    </div>
                                                    <!-- Contractor Name Field (Populated dynamically based on project selection) -->
                                                    <div class="col-4">
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT contractor_name FROM project WHERE project.contract_number = (SELECT permits.contract_number FROM permits WHERE permits.permit_id = ?)";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            ...
                                                            // Display project name in readonly input field
                                                            echo '<input type="text" name="contractor_name" id="contractor_name" value="' . $contractorName . '" class="form-control text-center text-" readonly disabled="disabled" bg-secondary>';
                                                        } else {
                                                            echo "نام پیمانکار پروژه ثبت نشده است";
                                                        }
                                                        ?>
                                                    </div>

                                                    <!-- Contract Number Field (Populated dynamically based on project selection) -->
                                                    <div class="col-4">
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT contract_number FROM project WHERE project.contract_number = (SELECT permits.contract_number FROM permits WHERE permits.permit_id = ?)";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            ...
                                                            // Display project name in readonly input field
                                                            echo '<input type="text" name="contract_number" id="contract_number" value="' . $contract_number . '" class="form-control text-center text-" readonly disabled="disabled" bg-secondary>';
                                                        } else {
                                                            echo "شماره قرارداد پروژه ثبت نشده است";
                                                        }
                                                        ?>
                                                    </div>
                                                </div>
                                        ...
                                        <fieldset>
                                            <legend class="mb-10 text-muted">دیسیپلین / های پرمیت جاری</legend>
                                            <div class="container">
                                                <div class="row">
                                                    <!-- Discipline Group dropdown -->
                                                    <div class="col-4 fv-row mb-10">
                                                        <label class="d-flex align-items-center fs-5 fw-bold mb-2">
                                                            <span class="required">گروه دیسیپلین</span>
                                                        </label>
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT discipline_group FROM permits WHERE permits.permit_id = ? ";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            ...
                                                            // Display project name in readonly input field
                                                            echo '<input type="text" name="discipline_group" id="discipline_group" value="' . $discipline_group . '" class="form-control text-center text-" readonly disabled="disabled" bg-secondary>';
                                                        } else {
                                                            echo "هیچ گروه دیسیپلین پرمیت ای ثبت نشده است";
                                                        }
                                                        ?>
                                                    </div>
                                                    <!--begin::Discipline dropdown-->
                                                    <div class="col-4 fv-row mb-10">
                                                        <label class="d-flex align-items-center fs-5 fw-bold mb-2">
                                                            <span class="required">آیتم های دیسیپلین</span>
                                                        </label>
                                                        <?php
                                                        session_start();
                                                        // Database connection parameters
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT discipline FROM permits WHERE permits.permit_id = ? ";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            ...
                                                            $_SESSION['permitItemsDiscipline'] = $discipline;
                                                            // Display project name in readonly input field
                                                            echo '<input type="text" name="discipline" id="discipline" value="' . $discipline . '" class="form-control text-center text-" readonly disabled="disabled" bg-secondary>';
                                                        } else {
                                                            echo "هیچ آیتم دیسیپلینی ثبت نشده است";
                                                        }
                                                        ?>
                                                    </div>
                                                    <!-- Permit ID Field (Populated dynamically based on selected discipline) -->
                                                    <div class="col-4">
                                                        <label for="permit_id" class="form-label">شماره پرمیت</label>
                                                        <input type="text" name="permit_id" id="permit_id" class="form-control text-center" value="<?php echo $_GET['prmQWERTY']; ?>" readonly>
                                                    </div>
                                                </div>
                                            </div>
                                        </fieldset>
                                        <fieldset>
                                            <legend class="mb-10 text-muted">سایر اطلاعات پرمیت</legend>
                                            <div class="container">
                                                <div class="row">
                                                    <!-- Crew Input -->
                                                    <div class="col-4">
                                                        <label for="crew" class="form-label required">پیمانکار</label>
                                                        <?php
                                                        // Database connection parameters
                                                        $host = "localhost";
                                                        $username = "root";
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT crew FROM permits WHERE permits.permit_id = ? ";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            ...
                                                            // Display project name in readonly input field
                                                            echo '<input type="text" name="crew" id="crew" value="' . $crew . '" class="form-control text-center text-" readonly disabled="disabled" bg-secondary>';
                                                        } else {
                                                            echo "هیچ آیتم دیسیپلینی ثبت نشده است";
                                                        }
                                                        ?>
                                                    ...
                                                    <!-- Axis Input -->
                                                    <div class="col-4">
                                                        <label for="axis" class="form-label required">محور</label>
                                                        <?php
                                                        // Database connection parameters
                                                        $host = "localhost";
                                                        $username = "root";
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT axis FROM permits WHERE permits.permit_id = ? ";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            ...
                                                            // Display project name in readonly input field
                                                            echo '<input type="text" name="axis" id="axis" value="' . $axis . '" class="form-control text-center text-" readonly disabled="disabled" bg-secondary>';
                                                        } else {
                                                            echo "هیچ آیتم دیسیپلینی ثبت نشده است";
                                                        }
                                                        ?>
                                                    ...
                                                    <!-- Floor Input -->
                                                    <div class="col-4">
                                                        <label for="floor" class="form-label required">طبقه</label>
                                                        <?php
                                                        // Database connection parameters
                                                        $host = "localhost";
                                                        $username = "root";
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT floor FROM permits WHERE permits.permit_id = ? ";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            ...
                                                            // Display project name in readonly input field
                                                            echo '<input type="text" name="floor" id="floor" value="' . $floor . '" class="form-control text-center text-" readonly disabled="disabled" bg-secondary>';
                                                        } else {
                                                            echo "هیچ آیتم دیسیپلینی ثبت نشده است";
                                                        }
                                                        ?>
                                                    ...
                                                    <div class="col-4 mt-5">
                                                        <label for="block" class="form-label required">بلوک</label>
                                                        <?php
                                                        // Database connection parameters
                                                        $host = "localhost";
                                                        $username = "root";
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT `block` FROM permits WHERE permits.permit_id = ? ";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            ...
                                                            // Display project name in readonly input field
                                                            echo '<input type="text" name="block" id="block" value="' . $block . '" class="form-control text-center text-" readonly disabled="disabled" bg-secondary>';
                                                        } else {
                                                            echo "هیچ آیتم دیسیپلینی ثبت نشده است";
                                                        }
                                                        ?>
                                                    ...
                                                    <!-- Zone Input -->
                                                    <div class="col-4 mt-5">
                                                        <label for="zone" class="form-label">زون</label>
                                                        <?php
                                                        // Database connection parameters
                                                        $host = "localhost";
                                                        $username = "root";
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT zone FROM permits WHERE permits.permit_id = ? ";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            ...
                                                            // Display project name in readonly input field
                                                            echo '<input type="text" name="zone" id="zone" value="' . $zone . '" class="form-control text-center text-" readonly disabled="disabled" bg-secondary>';
                                                        } else {
                                                            echo "هیچ آیتم دیسیپلینی ثبت نشده است";
                                                        }
                                                        ?>
                                                    ...
                                                    <!-- Time to Action Input -->
                                                    <div class="col-4 mt-5">
                                                        <label for="time_to_action" class="form-label required">پیش بینی مدت زمان اجرا</label>
                                                        <?php
                                                        // Database connection parameters
                                                        $host = "localhost";
                                                        $username = "root";
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT time_to_action FROM permits WHERE permits.permit_id = ? ";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            // Fetch and store project name
                                                            $row = $result->fetch_assoc();
                                                            $time_to_action = $row['time_to_action'];
                                                            // Remove "days" or "hours" and extract the numeric part
                                                            $datetime_departed = 0;
                                                            if (strpos($time_to_action, 'days') !== false) {
                                                                $datetime_departed = (int)str_replace('days', '', $time_to_action);
                                                                $unit = 'روز';
                                                            } elseif (strpos($time_to_action, 'hours') !== false) {
                                                                $datetime_departed = (int)str_replace('hours', '', $time_to_action);
                                                                $unit = 'ساعت';
                                                            }
                                                            // Display project name in readonly input field
                                                            echo '<input type="text" style="direction: rtl;" name="time_to_action" id="time_to_action" value="' . $datetime_departed . " " . $unit . '" class="form-control text-center text-" readonly disabled="disabled" bg-secondary>';
                                                        } else {
                                                            echo "هیچ آیتم دیسیپلینی ثبت نشده است";
                                                        }
                                                        ?>
                                                    ...
                                                    <div class="col-12 mb-5 mt-5">
                                                        <label class="form-label required" for="operation_description">توضیحات عملیات</label>
                                                        <?php
                                                        // Database connection parameters
                                                        $host = "localhost";
                                                        $username = "root";
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT operation_description FROM permits WHERE permits.permit_id = ? ";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            ...
                                                            // Display project name in readonly input field
                                                            echo '<input type="text" name="operation_description" id="operation_description" value="' . $operation_description . '" class="form-control text-center text-" readonly disabled="disabled" bg-secondary>';
                                                        } else {
                                                            echo "هیچ آیتم دیسیپلینی ثبت نشده است";
                                                        }
                                                        ?>
                                                    ...
                                                    <div class="col mt-5">
                                                        <label for="file_upload" class="form-label required">آپلود فایل</label><br />
                                                        ...
                                                        // Prepare and execute SQL query to fetch project name
                                                        $sql = "SELECT dir, file_name FROM permits WHERE permits.permit_id = ?";
                                                        $stmt = $conn->prepare($sql);
                                                        // Check if GET parameter 'prmQWERTY' is set, otherwise provide a default value
                                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : 'default_permit_id';
                                                        $stmt->bind_param("s", $permitId);
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            ...
                                                            // Split the concatenated file names into an array
                                                            ...
                                                            foreach ($file_names as $file) {
                                                                $full_path = $dir . $file;
                                                                echo '<div class="col-md-4 col-sm-12 col-xs-12 mt-5 mt-2">';
                                                                // Show file button
                                                                echo '<button type="button" class="w-100 col-md-6 col-sm-12 btn btn-dark btn-lg mb-3" data-bs-toggle="modal" data-bs-target="#fileModal" data-file="' . $full_path . '">نمایش فایل شماره ' . $counter . '</button>';
                                                                // Add download link
                                                                echo ' <a href="' . $full_path . '" class="w-100 col-md-6 col-sm-12 btn btn-success btn-lg mb-5" download>دانلود فایل شماره ' . $counter . '</a>';
                                                                echo '</div>';
                                                                $counter++;
                                                            }
                                                            echo '</div>';
                                                        } else {
                                                            echo "هیچ آیتمي یافت نشد";
                                                        }
                                                        ...
                                                        <div class="modal-dialog modal-lg">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title" id="fileModalLabel">پیش نمایش فایل</h5>
                                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                                </div>
                                                                <div class="modal-body">
                                                                    <iframe id="modalFile" src="" style="height: 400px; width: 100%;"></iframe>
                                                                </div>
                                                    ...
                                                    <script>
                                                        document.addEventListener('DOMContentLoaded', function() {
                                                            ...
                                                            fileModal.addEventListener('show.bs.modal', function(event) {
                                                                ...
                                                                var modalFile = fileModal.querySelector('#modalFile');
                                                                modalFile.src = fileSrc;
                                                            });
                                                        });
                                                    </script>
                            ...
                            <!-- Tab #1: Complete Latest Activity -->
                            <div class="accordion-item mb-5 col-md-12 col-sm-12 rounded-2">
                                <h2 class="accordion-header" id="tab1-heading">
                                    <button class="accordion-button text-black" type="button" data-bs-toggle="collapse" data-bs-target="#tab1-collapse2" aria-expanded="true" aria-controls="tab1-collapse">
                                        تکمیل فعالیت پیشین
                                    </button>
                                </h2>
                                <div id="tab1-collapse2" class="accordion-collapse collapse" aria-labelledby="tab1-heading" data-bs-parent="#accordion">
                                    <div class="accordion-body">
                                        <?php
                                        error_reporting(E_ALL);
                                        ...
                                        // Fetch project last activity from database based on project ID
                                        $getDiscipline = "SELECT last_activity FROM permits WHERE permit_id = ?";
                                        $stmt = $conn->prepare($getDiscipline);
                                        ...
                                        // Bind parameters
                                        $permitId = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : '';
                                        $stmt->bind_param("s", $permitId);
                                        ...
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                $checkType = substr($lastActivity, 4, 1);
                                                $typeOfAction = ($checkType === "P" ? "Permit" : "Checklist");
                                                if (!empty($lastActivity)) {
                                                    echo ($typeOfAction === "Permit" ? "پرمیت شماره : " : "چک لیست شماره :");
                                                    echo '<label class="form-check-label p-3 pr-3">
                                                                <input type="checkbox" name="items[]" value="' . htmlspecialchars($lastActivity, ENT_QUOTES, 'UTF-8') . '" class="form-check-input">
                                                                &nbsp;' . htmlspecialchars($lastActivity, ENT_QUOTES, 'UTF-8') . '
                                                            </label>';
                                                    echo '<a target="_blank" href="./view_Disciplines.php?itemID=' . $lastActivity . ' ">مشاهده آیتم های فعالیت پیشین</a>';
                                                }
                                            }
                                        } else {
                                            // Handle the case where no results are found
                                            echo "هیچ موردی برای تکمیل فعالیت پیشین یافت نشد.";
                                        }
                            ...
                            <!-- Tab #2: Documents -->
                            <div class="accordion-item mb-5 col-md-12 col-sm-12 rounded-2">
                                <h2 class="accordion-header" id="tab2-heading">
                                    <button class="accordion-button collapsed text-black" type="button" data-bs-toggle="collapse" data-bs-target="#tab2-collapse" aria-expanded="false" aria-controls="tab2-collapse">
                                        مستندات
                                    </button>
                                </h2>
                                <div id="tab2-collapse" class="accordion-collapse collapse" aria-labelledby="tab2-heading" data-bs-parent="#accordion">
                                    <div class="accordion-body">
                                        <?php
                                        error_reporting(E_ALL);
                                        // Database connection parameters
                                        ...
                                        $getDiscipline = "SELECT `documentations` FROM `permitItemsSelection` WHERE `discipline_id` = (SELECT `discipline_id` FROM `permitItemsSelectionDone` WHERE `permit_id` = ?)";
                                        $stmt = $conn->prepare($getDiscipline);
                                        $stmt->bind_param("s", $_GET['prmQWERTY']);
                                        $stmt->execute();
                                        $result = $stmt->get_result();
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                $documentations = $row['documentations'];
                                                if ($documentations) {
                                                    echo '<label class="form-check-label p-3">
                                                                <input type="checkbox" name="items[]" value="' . $documentations . '" class="form-check-input">
                                                                &nbsp;' . $documentations . '
                                                            </label><br />';
                                                } else {
                                                    echo "";
                                            ...
                                            echo "هیچ موردی برای مستندات یافت نشد.";
                                            // exit();
                                        }
                                        ?>
                                    </div>
                            ...
                            <!-- Tab #3: Work Materials -->
                            <div class="accordion-item mb-5 col-md-12 col-sm-12 rounded-2">
                                <h2 class="accordion-header" id="tab3-heading">
                                    <button class="accordion-button collapsed text-black" type="button" data-bs-toggle="collapse" data-bs-target="#tab3-collapse" aria-expanded="false" aria-controls="tab3-collapse">
                                        مصالح مورد نیاز
                                    </button>
                                </h2>
                                <div id="tab3-collapse" class="accordion-collapse collapse" aria-labelledby="tab3-heading" data-bs-parent="#accordion">
                                    <div class="accordion-body">
                                        <?php
                                        error_reporting(E_ALL);
                                        // Database connection parameters
                                        ...
                                        // Fetch project name from database based on project ID
                                        $getDiscipline = "SELECT `workingMaterials` FROM `permitItemsSelection` WHERE `discipline_id` = (SELECT `discipline_id` FROM `permitItemsSelectionDone` WHERE `permit_id` = ?)";
                                        $stmt = $conn->prepare($getDiscipline);
                                        $stmt->bind_param("s", $_GET['prmQWERTY']);
                                        $stmt->execute();
                                        $result = $stmt->get_result();
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                $workingMaterials = $row['workingMaterials'];
                                                if ($workingMaterials) {
                                                    echo '<label class="form-check-label p-3">
                                                                <input type="checkbox" name="items[]" value="' . $workingMaterials . '" class="form-check-input">
                                                                &nbsp;' . $workingMaterials . '
                                                            </label><br />';
                                                } else {
                                                    echo "";
                                            ...
                                            // Handle the case where no project is found
                                            echo "هیچ موردی برای مصالح مورد یافت نشد.";
                                            // exit();
                                        }
                                        ?>
                                    </div>
                            ...
                            <!-- Tab #4: Tools -->
                            <div class="accordion-item mb-5 col-md-12 col-sm-12 rounded-2">
                                <h2 class="accordion-header" id="tab4-heading">
                                    <button class="accordion-button collapsed text-black" type="button" data-bs-toggle="collapse" data-bs-target="#tab4-collapse" aria-expanded="false" aria-controls="tab4-collapse">
                                        ابزارآلات و تجهیزات
                                    </button>
                                </h2>
                                <div id="tab4-collapse" class="accordion-collapse collapse" aria-labelledby="tab4-heading" data-bs-parent="#accordion">
                                    <div class="accordion-body">
                                        <?php
                                        error_reporting(E_ALL);
                                        // Database connection parameters
                                        ...
                                        // Fetch project name from database based on project ID
                                        $getDiscipline = "SELECT `tools` FROM `permitItemsSelection` WHERE `discipline_id` = (SELECT `discipline_id` FROM `permitItemsSelectionDone` WHERE `permit_id` = ?)";
                                        $stmt = $conn->prepare($getDiscipline);
                                        $stmt->bind_param("s", $_GET['prmQWERTY']);
                                        $stmt->execute();
                                        $result = $stmt->get_result();
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                if ($tools) {
                                                    echo '<label class="form-check-label p-3">
                                                                <input type="checkbox" name="items[]" value="' . $tools . '" class="form-check-input">
                                                                &nbsp;' . $tools . '
                                                            </label><br />';
                                                } else {
                                                    echo "";
                                            ...
                                            // Handle the case where no project is found
                                            echo "هیچ موردی برای ابزار آلات و تجهیزات یافت نشد.";
                                            // exit();
                                        }
                                        ?>
                                    </div>
                            ...
                            <!-- Tab #5: Environment Conditions -->
                            <div class="accordion-item mb-5 col-md-12 col-sm-12 rounded-2">
                                <h2 class="accordion-header" id="tab5-heading">
                                    <button class="accordion-button collapsed text-black" type="button" data-bs-toggle="collapse" data-bs-target="#tab5-collapse" aria-expanded="false" aria-controls="tab5-collapse">
                                        شرایط محیطی
                                    </button>
                                </h2>
                                <div id="tab5-collapse" class="accordion-collapse collapse" aria-labelledby="tab5-heading" data-bs-parent="#accordion">
                                    <div class="accordion-body">
                                        <?php
                                        error_reporting(E_ALL);
                                        // Database connection parameters
                                        ...
                                        // Fetch project name from database based on project ID
                                        $getDiscipline = "SELECT `env` FROM `permitItemsSelection` WHERE `discipline_id` = (SELECT `discipline_id` FROM `permitItemsSelectionDone` WHERE `permit_id` = ?)";
                                        $stmt = $conn->prepare($getDiscipline);
                                        $stmt->bind_param("s", $_GET['prmQWERTY']);
                                        $stmt->execute();
                                        $result = $stmt->get_result();
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                if ($env) {
                                                    echo '<label class="form-check-label p-3">
                                                                <input type="checkbox" name="items[]" value="' . $env . '" class="form-check-input">
                                                                &nbsp;' . $env . '
                                                            </label><br />';
                                                } else {
                                                    echo "";
                                            ...
                                            // Handle the case where no project is found
                                            echo "هیچ موردی برای شرایط محیطی یافت نشد.";
                                            // exit();
                                        }
                                        ?>
                                    </div>
                            ...
                            <!-- Tab #6: HSE -->
                            <div class="accordion-item mb-5 col-md-12 col-sm-12 rounded-2">
                                <h2 class="accordion-header" id="tab6-heading">
                                    <button class="accordion-button collapsed text-black" type="button" data-bs-toggle="collapse" data-bs-target="#tab6-collapse" aria-expanded="false" aria-controls="tab6-collapse">
                                        HSE
                                    </button>
                                </h2>
                                <div id="tab6-collapse" class="accordion-collapse collapse" aria-labelledby="tab6-heading" data-bs-parent="#accordion">
                                    <div class="accordion-body">
                                        <?php
                                        error_reporting(0);
                                        // Database connection parameters
                                        ...
                                        // Fetch project name from database based on project ID
                                        $getDiscipline = "SELECT `hse` FROM `permitItemsSelection` WHERE `discipline_id` = (SELECT `discipline_id` FROM `permitItemsSelectionDone` WHERE `permit_id` = ?)";
                                        $stmt = $conn->prepare($getDiscipline);
                                        $stmt->bind_param("s", $_GET['prmQWERTY']);
                                        $stmt->execute();
                                        $result = $stmt->get_result();
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                if ($hse) {
                                                    echo '<label class="form-check-label p-3">
                                                                <input type="checkbox" name="items[]" value="' . $hse . '" class="form-check-input">
                                                                &nbsp;' . $hse . '
                                                            </label><br />';
                                                } else {
                                                    echo "";
                                            ...
                                            // Handle the case where no project is found
                                            echo "هیچ موردی برای HSE یافت نشد.";
                                            // exit();
                                        }
                                        ?>
                                    </div>
                            ...
                            <div class="form-group mt-2 p-0">
                                <textarea id="description" name="description" class="form-control" rows="4" cols="100" placeholder="توضیحات"></textarea>
                            </div>

                            <!-- Accept and Reject Radio Buttons -->
                            <div class="form-group mt-10 text-white p-5 bg-white rounded-2">
                                <label class="d-block mb-5 text-black">آیا فرم جاری مورد تایید می باشد ؟</label>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="permit_status" id="accept" value="Accept">
                                    <label class="form-check-label text-primary" for="accept">مورد تایید می باشد</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="permit_status" id="reject" value="Reject">
                                    <label class="form-check-label text-danger" for="reject">مورد تایید نمی باشد</label>
                                </div>
                            </div>
                            ...
                            <script>
                                document.addEventListener('DOMContentLoaded', function() {
                                    ...
                                    // Function to toggle required attribute based on radio button selection
                                    ...
                                    acceptRadio.addEventListener('change', checkValidation);
                                    rejectRadio.addEventListener('change', checkValidation);
                                });
                            </script>
                        </div>
                    </div>
                    <input type="hidden" name="prmQWERTY" value="<?php echo isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : ''; ?>">
                    <?php
                    ...
                    $query1 = "SELECT * FROM users WHERE username = ? AND job_position = 3";
                    $stmt1 = $conn->prepare($query1);
                    $stmt1->bind_param("s", $current_username);
                    $stmt1->execute();
                    $result1 = $stmt1->get_result();
                    $is_manager = $result1->num_rows > 0 ? "manager" : "not manager";

                    // Check if the current user is one of the owners in the projectPersonnel table
                    $query2 = "SELECT * FROM projectPersonnel WHERE userID = (SELECT id FROM users WHERE username = ?) AND role = 'owner'";
                    $stmt2 = $conn->prepare($query2);
                    $stmt2->bind_param("s", $current_username);
                    $stmt2->execute();
                    $result2 = $stmt2->get_result();
                    $is_owner = $result2->num_rows > 0;

                    // Check if all other owners have submitted the permitItems
                    $permitID = isset($_GET['prmQWERTY']) ? $_GET['prmQWERTY'] : '';
                    $query3 = "SELECT userID 
            ...
            AND projectID = (
                ...
                LIMIT 1 )";
                    ...
                    $stmt3->bind_param("s", $permitID);
                    $stmt3->execute();
                    $result3 = $stmt3->get_result();

                    // Initialize a flag to track if all owners have submitted

                    // Loop through each owner
                    while ($row3 = $result3->fetch_assoc()) {
                        $all_owners_submitted = true;
                        $query4 = "SELECT * FROM `permit_status` WHERE `permit_id` = ? AND `user_id` != ? ";
                        $stmt4 = $conn->prepare($query4);
                        $stmt4->bind_param("ss", $permitID, $_SESSION['username']);
                        $stmt4->execute();
                        $result4 = $stmt4->get_result();
                        // Get all users count
                        $query5 = "SELECT COUNT(*) AS owner_count
            FROM projectPersonnel   
            WHERE projectPersonnel.role = 'owner'
            AND projectPersonnel.userID IN (
                SELECT userID
                FROM projectPersonnel
                WHERE projectID IN (
                    SELECT projectID
            ...
            ) AND projectPersonnel.userID NOT IN (
                SELECT id
                FROM users
                WHERE job_position = 3)";
                        'username:' . $_SESSION['username'];
                        $stmt5 = $conn->prepare($query5);
                        $stmt5->bind_param("s", $_SESSION['username']);
                        $stmt5->execute();
                        $result5 = $stmt5->get_result();
                        $row5 = $result5->fetch_assoc();
                        $owner_count = $row5['owner_count'];

                        // If any owner has not submitted, set the flag to false and break the loop
                        if ($result4->num_rows != $owner_count) {
                            $all_owners_submitted = false;
                            break; // Exit the loop if any owner has not submitted
                        }
                    }
                    // Check if the current user is a manager and one of the owners, and if all other owners have submitted the permitItems
                    // if ($is_manager == "manager" &&
                    // $is_owner == 1 && $all_owners_submitted == 1) {
                    if ($is_manager == "manager" && $is_owner == 1) {
                        // Display the form with the submit button
                        echo "<div class='container-fluid mt-5 mb-5'>";
                        echo "<div class='row'>";
                        echo '<div class="col-12">';
                        // echo '<div class="container-fluid mt-5 p-0">
                        //             <div class="col-6 mt-3 mb-3">
                        //                 <label class="form-check-label text-white" for="permit_credit">مدت زمان اعتبار پرمیت</label>
                        //             </div>        
                        //             <div class="row">
                        //                 <div class="col">
                        //                     <input data-jdp class="form-control" id="permit_credit" name="permit_credit" />
                        //                 </div>
                        //             </div>
                        //         </div>';
                        echo '<div class="col-xs-12 col-sm-12 col-md-6 mt-5">
                                    <label for="permit_credit_unit" class="form-label text-white">مدت زمان اعتبار پرمیت</label>
                                    <select name="permit_credit_unit" id="permit_credit_unit" class="form-control text-center" onchange="toggleTimeInput()">
                                        <option value="days">تعداد روز</option>
                                        <option value="hours">ساعت</option>
                                    </select>
                                    <input type="number" name="permit_credit" id="permit_credit" class="form-control text-center" min="0">
                                </div>';
                        ...
                        echo '<div class="form-group mt-10 text-white">
                                <label class="d-block mb-5 text-white">آیا نیاز به پروتکشن می باشد ؟</label>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="protection" id="yes" value="Yes">
                                    <label class="form-check-label" for="yes">بلی، نیاز دارد.</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="protection" id="no" value="No">
                                    <label class="form-check-label" for="no">خیر، نیاز ندارد.</label>
                                </div>
                            </div>';
                        ...
                        echo "<div class='col-3'>&nbsp;</div>";
                        echo "<div class='col-md-6 col-sm-12 col-xs-12 mb-5'>";
                        echo '<button type="submit" class="w-100 btn btn-white text-dark mt-15">ارسال فرم پرمیت</button>';
                        echo "</div>";
                        echo "</div>";
                        echo "</div>";
                    }
                    if ($is_manager == "not manager") {
                        echo "<div class='container-fluid mb-5'>";
                        echo "<div class='row'>";
                        echo "<div class='col-3'>&nbsp;</div>";
                        echo "<div class='col-3'>&nbsp;</div>";
                        echo "<div class='col-6'>";
                        echo '<button type="submit" class="btn btn-white text-dark mt-15 w-100">ارسال فرم پرمیت جاری</button>';
                        echo "</div>";
                        echo "</div>";
                        echo "</div>";
                    }
                    ...
                    //     echo "<div class='col-3'>&nbsp;</div>";
                    //     echo "<div class='col-6'>";
                    //     echo "<p class='btn btn-dark mt-15 w-100'>شما مجاز به ارسال این فرم نیستید.</p>";
                    //     echo "</div>";
                    //     echo "</div>";
                    //     echo "</div>";
                    // }
                    ...
                    <script>
                        ...
                        document.getElementById('permit_day').addEventListener('change', function() {
                            setSession('permit_day', this.value); // Call setSession function when day changes
                        });

                        document.getElementById('permit_month').addEventListener('change', function() {
                            setSession('permit_month', this.value); // Call setSession function when month changes
                        });

                        document.getElementById('permit_year').addEventListener('change', function() {
                            setSession('permit_year', this.value); // Call setSession function when year changes
                        });

                        // Function to send AJAX request and set session variable
                        function setSession(key, value) {
                            ...
                            xhr.open('GET', 'set_session.php?key=' + key + '&value=' + value, true);
                            xhr.send();
                        }
                    </script>
            </div>
    ...
    <!--begin date picker   -->
    <script src="../../assets/js/jalaly-date-picker/jalalidatepicker.js"></script>
    <script>
        jalaliDatepicker.startWatch();
        jalaliDatepicker.persianDigits();
    </script>
    <!--end date picker   -->
    <!--end::Javascript-->
</body>
