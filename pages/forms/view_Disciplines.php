...
-->
            ...
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
                ...
                // دریافت پارامتر itemID از URL
                $itemID = $_GET['itemID'] ?? '';

                // دریافت اطلاعات پروژه و چک لیست بر اساس itemID
                $sqlProject = "
    SELECT 
        checklist.projectname AS 'نام پروژه',
        checklist.checklistID AS 'شماره چک لیست',
        checklist.contractor_name AS 'نام پیمانکار',
        checklist.operation_description AS 'توضیحات عملیات',
        checklist.crew AS 'تیم پیمانکار',
        checklist.axis AS 'محور',
        checklist.block AS 'بلوک',
        checklist.zone AS 'زون',
        checklist.discipline AS 'دیسیپلین',
...
";
                ...
                // نمایش اطلاعات پروژه و چک لیست
                echo '
    <div class="container mt-4 w-75" dir="RTL">
        <h2 class="mb-4 text-white pt-10">اطلاعات پروژه و چک لیست</h2>';
            ...
            <div class="card-body">
                ...
                <p><strong>نام پروژه:</strong> ' . htmlspecialchars($projectData['نام پروژه']) . '</p>
                <p><strong>شماره چک لیست:</strong> ' . htmlspecialchars($projectData['شماره چک لیست']) . '</p>
                <p><strong>نام پیمانکار:</strong> ' . htmlspecialchars($projectData['نام پیمانکار']) . '</p>
                <p><strong>توضیحات عملیات:</strong> ' . htmlspecialchars($projectData['توضیحات عملیات']) . '</p>
                <p><strong>تیم پیمانکار:</strong> ' . htmlspecialchars($projectData['تیم پیمانکار']) . '</p>
                <p><strong>زون:</strong> ' . htmlspecialchars($projectData['زون']) . '</p>
                <p><strong>بلوک:</strong> ' . htmlspecialchars($projectData['بلوک']) . '</p>
                <p><strong>محور:</strong> ' . htmlspecialchars($projectData['محور']) . '</p>
                <p><strong>دیسیپلین:</strong> ' . htmlspecialchars($projectData['دیسیپلین']) . '</p>
                <p><strong>وضعیت چک لیست:</strong> ' . getStatusBadge(htmlspecialchars($projectData['status'])) . '</p>
            </div>
        </div>';
                } else {
                    echo '<div class="alert alert-warning">هیچ اطلاعاتی یافت نشد.</div>';
                }
                ...
                function getStatusBadge($status)
                {
                    switch ($status) {
                        case "issued":
                            return '<span class="badge bg-warning text-white">ایجاد شده</span>';
                        case "in progress":
                            return '<span class="badge bg-info text-white">در حال بررسی</span>';
                        case "Accept":
                            return '<span class="badge bg-success text-white">تایید شده</span>';
                        case "Reject":
                            return '<span class="badge bg-danger text-white">تایید نشده</span>';
                        case "N/A":
                            return '<span class="badge bg-secondary text-white">N/A</span>';
                        default:
                            return '<span class="badge bg-dark text-white">بدون وضعیت</span>';
                    }
                }

                // تعریف کوئری برای انتخاب داده‌ها بدون ستون‌های خاص
                $sqlItems = "
    SELECT 
        operation_description AS 'عنوان عملیات',
        review_criteria AS 'معیارهای بازبینی',
        control_method_tool AS 'روش / ابزار کنترل',
        minimum_check_value AS 'حداقل مقدار بررسی',
        considerations AS 'ملاحظات',
        allowable_tolerance AS 'تفاوت مجاز'
    FROM 
        checklist_items
    WHERE 
        checklist_items.discipline_id = (
            SELECT disciplines.id
            FROM disciplines
            INNER JOIN checklist 
                ON disciplines.name COLLATE utf8mb4_general_ci = checklist.discipline COLLATE utf8mb4_general_ci
...
";
                ...
                $stmtItems->bind_param("s", $itemID);
                $stmtItems->execute();
                $resultItems = $stmtItems->get_result();

                // نمایش داده‌ها
                echo '<div class="container bg-white p-10 rounded-3"><h2 class="mb-4 text-black pt-10">آیتم های چک لیست</h2>';
            ...
            <table class="table table-striped">
                <thead class="text-black">
                    ...
                    // نمایش عنوان ستون‌ها به صورت داینامیک، بدون ستون‌های id و discipline_id
                    $fields = $resultItems->fetch_fields();
                ...
                <tbody>';
                    ...
                    // نمایش داده‌های هر ردیف، بدون ستون‌های id و discipline_id
                    $resultItems->data_seek(0); // reset pointer to fetch all rows
