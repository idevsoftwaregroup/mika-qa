...
$title = "Quality Assurance | Owner Dashboard";
...
-->
                                            ...
                                            <div class="m-0">
                                                ...
                                                <div class="mb-10">
                                                    ...
                                                    <br />
                                                    <!--begin::Item-->
                                                    <a href="/pages/forms/projectsOwner.php" class="custom-list d-flex align-items-center px-5 py-1">
                                                        <div class="d-flex flex-column flex-grow-1">
                                                            <!--begin::Link-->
                                                    ...
                                                    //                                                         if ($jobPosition == 3) {
                                                    //                                                             echo '
                                                    //                 <a href="/historyOwner.php" class="custom-list d-flex align-items-center px-5 py-1">
                                                    //                                                         <div class="d-flex flex-column flex-grow-1">
                                                    //                                                             <!--begin::Link-->
                                                    // <span id="history" class="text-black fw-bold bg-white" style="border: 1px solid white;padding: 10px;border-radius: 5px;">هیستوری فرآیندها</span></div>
                ...
                <div id="kt_header" class="header" data-kt-sticky="true" data-kt-sticky-name="header" data-kt-sticky-offset="{default: '200px', lg: '300px'}">
                    ...
                    <div class="container-fluid d-flex align-items-center justify-content-between" id="kt_header_container">
                        ...
                        <div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-2 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
                            <!--begin::Heading-->
                            <!-- <h1 class="text-dark fw-bolder my-0 fs-2">داشبورد کاربری ( کارفرما )</h1> -->
                            <!--end::Heading-->
                            <!--begin::Breadcrumb-->
                            <ul class="breadcrumb fw-bold fs-base my-1">
                                <li class="breadcrumb-item text-muted">
                                    <a href="/contractor.php" class="p-5 btn btn-white text-dark rounded" style="border: 1px solid white">صفحه نخست</a>
                                </li>
                                <li class="breadcrumb-item p-5 btn btn-black text-white rounded" style="border: 1px solid white">کارتابل کارفرما</li>
                ...
                <div class="container-fluid p-10" style="direction: rtl;overflow-x: scroll;">
                    <div class="row">
                            ...
                            <table class="table table-striped mb-10">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" class="text-white w-5">
                                        ...
                                        if ($rowFullnameResult) {
                                            ...
                                            // echo "<img  />";
                                            if ($jobPosition == 3) {
                                                echo '<th scope="col" class="text-white">
                                                            <!-- New Action Column Header -->
                                                            <input class="form-control form-control-sm" id="searchDate" type="text" placeholder="عملیات" disabled>
                                                        </th>';
                                            } else {
                                                echo "";
                                            }
                                ...
                                <tbody id="tableBody">
                                    ...
                                    echo $current_user_username = $_SESSION['username'];
                                    ...
                                    // Function to check if user is manager and owner
                                    function isUserManagerAndOwner($conn, $username)
                                    {
                                        $query = "
                                                    SELECT
                                                        u.id
                                                    FROM
                                                        users u
                                                    JOIN
                                                        projectPersonnel pp ON u.id = pp.userID
                                                    WHERE
                                                        u.username = ?
                                                        AND u.job_position = 3
                                                        AND pp.role = 'owner'
                                                ";
                                        $stmt = $conn->prepare($query);
                                        ...
                                        $stmt->execute();
                                        $result = $stmt->get_result();
                                        return $result->num_rows > 0;  // Return true if user is both manager and owner, false otherwise
                                    }
                                    ...
                                    {
                                        // $query = "SELECT * FROM permits WHERE project_name = (SELECT project_name FROM project WHERE id = ?) ORDER BY dateTimeCreated DESC;";
                                        $query = "SELECT * FROM permits 
                                                    WHERE project_name = (SELECT `project_name` FROM project WHERE id = ?) AND contractor_name = (SELECT `contractor_name` FROM project WHERE id = ?) AND permits.isDeleted is False OR permits.isDeleted is NULL
                                                    ORDER BY dateTimeCreated DESC";
                                        $stmt = $conn->prepare($query);
                                        $stmt->bind_param("ii", $_SESSION['project_id'], $_SESSION['project_id']);
                                        $stmt->execute();
                                        $result = $stmt->get_result();

                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                // Check if user is manager and owner or if the permit is not submitted by the user and its status is not "Accept", "Reject", "N/A"
                                                if (isUserManagerAndOwner($conn, $username) || (permitSubmittedByUser($conn, $row['permit_id'], $username) === 0 && !in_array($row["status"], ["Accept", "Reject", "N/A"]))) {
                                                    ...
                                                    echo "<tr>
                        <td class='col-3'><a class='btn btn-sm btn-dark text-white w-100 rounded' href='../pages/forms/permitItems.php?prmQWERTY=" . htmlspecialchars($row["permit_id"]) . "'>" . htmlspecialchars($row["permit_id"]) . "</a></td>
                        ...
                        <td>" . htmlspecialchars($jalali_date) . "</td>";
                                                    ...
                                                    if (isUserManagerAndOwner($conn, $username)) {
                                                        echo "<td><a class='btn btn-dark btn-sm w-100' href='../pages/forms/progress.php?type=permit&itemID={$row['permit_id']}'>وضعیت</a></td>";
                                                    } else {
                                                        echo "<td></td>";
                                                    }
                                    ...
                                    // Function to display checklists
                                    function displayChecklists($conn, $username)
                                    {
                                        // $query = "SELECT * FROM checklist WHERE projectname = (SELECT project_name FROM project WHERE id = ?) order by date_created desc";
                                        $query = "SELECT
    *
FROM
    checklist
WHERE
    ...
    WHERE
        id = ?
) AND checklist.isDeleted is NULL OR checklist.isDeleted = NULL OR checklist.isDeleted = False AND checklist.status = 'Accept'
ORDER BY
    date_created
DESC
    ";
                                        ...
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                // Check if user is manager and owner or if the checklist is not submitted by the user and its status is not "Accept", "Reject", "N/A"
                                                if (isUserManagerAndOwner($conn, $username) || (!checklistSubmittedByUser($conn, $row['permit_id'], $username) && !in_array($row["status"], ["Accept", "Reject", "N/A"]))) {
                                                    ...
                                                    // <td>" . htmlspecialchars($row["contractor_name"]) . "</td>
                                                    echo "<tr>
                        <td class='col-md-3'><a class='btn btn-sm btn-dark text-white rounded w-100' href='../pages/forms/checklistItems.php?prmQWERTY=" . htmlspecialchars($row["checklistID"]) . "'>" . htmlspecialchars($row["checklistID"]) . "</a></td>
                        ...
                        <td>" . htmlspecialchars($jalali_date) . "</td>";
                                                    ...
                                                    if (isUserManagerAndOwner($conn, $username)) {
                                                        echo "<td><a class='btn btn-dark btn-sm w-100' href='../pages/forms/progress.php?type=checklist&itemID={$row['checklistID']}'>وضعیت</a></td>";
                                                    } else {
                                                        echo "<td></td>";
                                                    }
                                            ...
                                            echo "<tr><td colspan='8' class='text-center'>هیچ چک لیست <span class='text-primary'>تایید شده </span>ای یافت نشد.</td></tr>";
                                        }
                                    }
